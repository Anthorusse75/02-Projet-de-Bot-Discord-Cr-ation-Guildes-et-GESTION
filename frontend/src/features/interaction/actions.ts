...
export const actions: readonly AppAction[] = [
  ...
  { id: 'CREATE_VARIANT', sourceTypes: ['TRANSLATION_GROUP'], targetTypes: ['LANGUAGE_TARGET'], requiresTarget: true, min: 1, max: 1, guildMode: 'SAME', sourceUserCapabilities: ['plans.create','structure.write'], sourceBotCapabilities: ['CREATE_CHANNEL'], providerCapabilities: ['ROUTING_SUPPORTED'], risk: 'MEDIUM', labelKey: 'actions.createVariant', descriptionKey: 'actions.createVariant.description', tooltipKey: 'actions.createVariant.tooltip', intention: 'PLAN' },
  { id: 'LINK_EXISTING_VARIANT', sourceTypes: ['TRANSLATION_GROUP'], targetTypes: ['LANGUAGE_TARGET'], requiresTarget: true, min: 1, max: 1, guildMode: 'SAME', sourceUserCapabilities: ['plans.create','structure.write'], risk: 'MEDIUM', labelKey: 'actions.linkVariant', descriptionKey: 'actions.linkVariant.description', tooltipKey: 'actions.linkVariant.tooltip', intention: 'PLAN' },
  { id: 'CLONE_UNLINKED', sourceTypes: ['TRANSLATION_GROUP'], targetTypes: ['GUILD'], requiresTarget: true, min: 1, max: 1, guildMode: 'CROSS', sourceUserCapabilities: ['structure.read'], destinationUserCapabilities: ['plans.create','structure.write'], destinationBotCapabilities: ['CREATE_CHANNEL'], risk: 'MEDIUM', labelKey: 'actions.cloneUnlinked', descriptionKey: 'actions.cloneUnlinked.description', tooltipKey: 'actions.cloneUnlinked.tooltip', intention: 'PORTABLE_CLONE' },
  { id: 'PREVIEW', sourceTypes: ['TRANSLATION_GROUP'], targetTypes: ['LANGUAGE_TARGET','GUILD'], min: 1, max: 1, guildMode: 'ANY', sourceUserCapabilities: ['structure.read'], risk: 'LOW', labelKey: 'actions.translationPreview', descriptionKey: 'actions.translationPreview.description', tooltipKey: 'actions.translationPreview.tooltip', intention: 'READ' },
] as const
