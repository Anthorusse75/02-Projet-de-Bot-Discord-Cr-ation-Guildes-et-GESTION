...
function destinationAt(x: number, y: number): ResourceRef | undefined {
  ...
  const parsedPosition = element.dataset.dropPosition === undefined ? undefined : Number(element.dataset.dropPosition)
  const parentId = element.dataset.dropParent === undefined ? undefined : element.dataset.dropParent || null
  return {
    id,
    guildId: guildId as ResourceRef['guildId'],
    type,
    name: element.dataset.dropName ?? id,
    ...(Number.isFinite(parsedPosition) ? { position: parsedPosition } : {}),
...
function flattenStructure(categories: Array<Channel & { channels: Channel[] }>, roots: Channel[]) {
  return [
...
export function StructureScreen() {
  ...
  const all = useMemo(
    () => query.data ? flattenStructure(query.data.categories, query.data.root_channels) : [],
    [query.data],
  )
  const byId = useMemo(() => new Map(all.map((item) => [item.id, item])), [all])
  const counts = useMemo(
    () => query.data ? countStructure(query.data.categories, query.data.root_channels) : { categories: 0, channels: 0, threads: 0 },
    [query.data],
  )
  ...
  useEffect(() => {
    ...
    setExpanded(new Set(query.data.categories.map((category) => category.id)))
  }, [query.data])

  useEffect(() => {
    if (!query.data || selection.length === 0) return
    const existing = new Set(all.map((item) => item.id))
  ...
  function pointerMove(event: ReactPointerEvent) {
    const dragging = manager.current.move(event.nativeEvent)
    if (!dragging || !gestureSource.current) return
  ...
  function pointerUp(event: ReactPointerEvent) {
    ...
    if (!resolution.valid) {
      announce(t('gesture.invalid'))
      return
    }
    if (result.kind === 'right-drag') {
      openMenu([result.source], result.x, result.y, 'drop', destination)
      return
    }
  ...
  async function execute(intent = previewIntent) {
    ...
    if (!availability?.enabled) {
      setProblemKey('errors.authorization.denied')
      return
    }
    setProblemKey(null)
    setFeedbackKey(null)
    try {
      ...
      if (destinationGuildId && destinationGuildId !== guild.guild_id) {
        await client.invalidateQueries({ queryKey: ['did', me.user.discord_user_id, destinationGuildId, 'dashboard-capabilities'] })
      }
    }
  }
  ...
  return (
    <section className="structure-explorer-screen">
      <header className="structure-explorer-heading">
        <div>
          <p className="eyebrow">{t('structure.explorerEyebrow')}</p>
          <h1>{t('structure.title')}</h1>
          <p className="structure-explorer-subtitle">{t('structure.explorerSubtitle')}</p>
        </div>
        <div className="structure-summary" aria-label={t('structure.title')}>
          <span>{t('structure.summary.categories', { count: counts.categories })}</span>
          <span>{t('structure.summary.channels', { count: counts.channels })}</span>
          <span>{t('structure.summary.threads', { count: counts.threads })}</span>
        </div>
      </header>
      ...
      <div className="structure-toolbar">
        ...
        <label className="structure-hidden-toggle">
          <input type="checkbox" checked={includeHiddenDeleted} onChange={(event) => setIncludeHiddenDeleted(event.target.checked)} />
          <span>{t('structure.showHiddenShort')}</span>
        </label>
        <button
          type="button"
          className="structure-action-button"
          disabled={selection.length === 0}
          onClick={(event) => openSelectionActions(event.currentTarget)}
        >
          <span aria-hidden="true">•••</span>
          {t('structure.selection.actions')}
        ...
        <div className="structure-workbench">
          <section className="structure-tree-panel" aria-label={t('a11y.tree')}>
            <div className="structure-panel-title">
              <div><span className="structure-panel-icon" aria-hidden="true">⌘</span><div><strong>{guild.name}</strong><small>{t('structure.current')}</small></div></div>
              <Badge tone={capabilities.freshness === 'FRESH' ? 'ok' : 'warning'}>{capabilities.freshness === 'FRESH' ? t('structure.current') : t('structure.stale')}</Badge>
            </div>
            ...
            {hasFilteredResults && (
              <Tree>
                {filteredCategories.map((category) => {
                  const childrenMatch = category.channels.filter((channel) => matches(channel) || channel.threads?.some(matches))
                  return (
                    <ResourceItem
                      key={category.id}
                      item={category}
                      selected={selection.some((item) => item.id === category.id)}
                      expanded={expanded.has(category.id)}
                      dropHoverKey={dragHoverKey}
                    ...
                    >
                      {childrenMatch.map((channel) => (
                        <ResourceItem
                          key={channel.id}
                          item={channel}
                          level={2}
                          selected={selection.some((item) => item.id === channel.id)}
                          expanded={expanded.has(channel.id)}
                          dropHoverKey={dragHoverKey}
                          onToggle={() => setExpanded((current) => {
                            const next = new Set(current)
                          ...
                          onPointerCancel={cancelGesture}
                        >
                          {channel.threads?.filter(matches).map((thread) => (
                            <ResourceItem
                              key={thread.id}
                              item={thread}
                              level={3}
                              selected={selection.some((item) => item.id === thread.id)}
                              expanded={false}
                              dropHoverKey={dragHoverKey}
                              onToggle={() => undefined}
                              onSelect={setSelection}
                ...
                {filteredRoots.length > 0 && (
                  <div className="structure-root-group">
                    <div className="structure-root-heading">
                      <span aria-hidden="true">⌂</span>
                      <span>{t('structure.rootLabel')}</span>
                    </div>
                    {filteredRoots.map((channel) => (
                      <ResourceItem
                        key={channel.id}
                        item={channel}
                        selected={selection.some((item) => item.id === channel.id)}
          ...
          <aside className="structure-inspector-panel">
            <Inspector
              selection={selection}
              item={selectedItem}
              capabilities={capabilities}
              onActions={openSelectionActions}
            />
          </aside>

          <aside className="structure-destinations-panel">
            <div className="structure-destination-heading">
              <p className="eyebrow">{t('structure.destinations.title')}</p>
              <p>{t('structure.destinations.subtitle')}</p>
            </div>
            ...
            <DropTargetCard
              ...
              title={t('structure.rootLabel')}
              subtitle={t('structure.drop.root')}
              icon="⌂"
            />
            <div className="destination-section-label">{t('structure.destinations.otherServers')}</div>
            <div className="destination-guild-list">
              {destinationGuilds.map((item) => (
                <DestinationGuildCard
                  key={item.guild_id}
                  guild={item}
                  userId={me.user.discord_user_id}
                  hoverKey={dragHoverKey}
                />
              ))}
            </div>
      ...
      {dragVisual && (
        <div className="structure-drag-overlay" style={{ transform: `translate3d(${dragVisual.x + 14}px, ${dragVisual.y + 14}px, 0)` }}>
          <span className="resource-kind-icon" aria-hidden="true">{dragVisual.source.type === 'CATEGORY' ? '▰' : '#'}</span>
          <div><strong>{dragVisual.source.name}</strong><small>{t(dragVisual.rightButton ? 'structure.drag.rightRelease' : dragHoverKey?.startsWith(`${guild.guild_id}:`) ? 'structure.drag.move' : 'structure.drag.copy')}</small></div>
        </div>
      )}

      {context && (
        <Menu labelKey={context.kind === 'drop' ? 'context.dropTitle' : 'context.title'} style={{ left: context.x, top: context.y }} onClose={() => setContext(null)}>
          {resolveActions(context).map(({ action, enabled, reasonKey }) => (
            <MenuItem key={action.id} disabled={!enabled} disabledReasonKey={reasonKey} onSelect={() => choose(action.id)}>{t(action.labelKey)}</MenuItem>
          ))}
        </Menu>
      )}

      <Dialog open={previewIntent !== null} titleKey="dialog.previewTitle" onClose={() => { setPreview(null); setProblemKey(null) }}>
        <div className="structure-preview-summary">
          <div><small>{t('structure.multiInspector', { count: previewIntent?.source.length ?? 0 })}</small><strong>{previewIntent?.source.map((item) => item.name).join(', ')}</strong></div>
          {previewIntent?.destination && <div><small>{t('actions.target')}</small><strong>{previewIntent.destination.name}</strong></div>}
        </div>
        <p>{t('dialog.noMutation')}</p>
        <p>{previewAction ? t(previewAction.descriptionKey) : null}</p>
        {previewAction?.requiresTarget && !previewIntent?.destination && (
          <Select labelKey="actions.target" value="" onChange={(event) => {
            const target = targetCandidates.find((item) => item.id === event.target.value)
            if (target && previewIntent) setPreview(createActionIntent(previewIntent.actionId, previewIntent.source, target))
          }}>
            <option value="">{t('actions.target.choose')}</option>
            {targetCandidates.map((item) => <option key={`${item.guildId}:${item.id}`} value={item.id}>{item.name}</option>)}
          </Select>
        )}
        ...
        <Button
          ...
          variant="primary"
          disabled={!previewAvailability?.enabled || Boolean(previewAction?.requiresTarget && !previewIntent?.destination)}
          disabledReasonKey={previewAvailability?.reasonKey ?? 'actions.disabled.target'}
...
function ResourceItem({ item, level = 1, selected, expanded, dropHoverKey, children, onToggle, onSelect, onMenu, onPointerDown, onPointerMove, onPointerUp, onPointerCancel }: ResourceItemProps) {
  ...
  function select(event: { ctrlKey?: boolean; metaKey?: boolean }) {
    const current = useInteractionStore.getState().selection
    onSelect(event.ctrlKey || event.metaKey
      ? selected ? current.filter((entry) => entry.id !== source.id) : [...current, source]
      : [source])
  }

  function keyboard(event: ReactKeyboardEvent<HTMLDivElement>) {
    if (!hasChildren) return
    if (event.key === 'ArrowRight' && !expanded) { event.preventDefault(); event.stopPropagation(); onToggle() }
    if (event.key === 'ArrowLeft' && expanded) { event.preventDefault(); event.stopPropagation(); onToggle() }
  }
  ...
  return (
    <TreeItem
      ...
      className={`structure-resource ${selected ? 'selected' : ''} ${dropHoverKey === key ? 'drop-hover' : ''} resource-${source.type.toLowerCase()}`}
      data-drop-id={item.id}
      data-drop-guild={item.guild_id}
      data-drop-name={item.name}
      data-drop-type={source.type}
      data-drop-position={item.position}
      data-drop-parent={item.parent_id ?? ''}
      onClick={(event) => { event.stopPropagation(); select(event) }}
      onKeyDown={keyboard}
    ...
    >
      <div className="structure-resource-row">
        <button
          type="button"
          className={`resource-expander ${hasChildren ? '' : 'placeholder'}`}
          tabIndex={-1}
          aria-label={hasChildren ? t(expanded ? 'structure.tree.collapse' : 'structure.tree.expand', { name: item.name }) : undefined}
        ...
        <span className="resource-copy">
          ...
          <small>{t(typeKey)} · {item.id}</small>
        </span>
        {source.type === 'CATEGORY' && item.resource_kind && <span className="resource-count">{(item as Channel & { channels?: Channel[] }).channels?.length ?? 0}</span>}
        {item.freshness !== 'FRESH' && <span className="resource-freshness-dot" title={t('common.stale')} />}
        <button
          type="button"
          className="resource-more"
          aria-label={t('structure.tree.actions')}
          onPointerDown={(event) => event.stopPropagation()}
          onClick={(event) => {
            event.stopPropagation()
            const rect = event.currentTarget.getBoundingClientRect()
            onMenu([source], rect.left, rect.bottom + 5, 'object')
          }}
        >•••</button>
      </div>
...
function Inspector({ selection, item, capabilities, onActions }: { selection: ResourceRef[]; item: Channel | undefined; capabilities: DashboardCapabilities; onActions: (anchor: HTMLElement) => void }) {
  ...
  if (selection.length === 0) {
    return <div className="inspector-empty"><span aria-hidden="true">◇</span><p>{t('structure.selection.none')}</p><small>{t('structure.selection.help')}</small></div>
  }
  if (selection.length > 1) {
    return (
      <div className="inspector-multi">
        <div className="inspector-title"><div><p className="eyebrow">{t('structure.properties')}</p><h2>{t('structure.multiInspector', { count: selection.length })}</h2></div><Badge>{selection.length}</Badge></div>
        <div className="inspector-selection-list">{selection.map((resource) => <div key={resource.id}><span className="resource-kind-icon" aria-hidden="true">{resource.type === 'CATEGORY' ? '▰' : '#'}</span><span>{resource.name}</span></div>)}</div>
        <button type="button" className="inspector-primary-action" onClick={(event) => onActions(event.currentTarget)}>{t('structure.selection.actions')}</button>
      </div>
    )
  }
  ...
  return (
    <div className="inspector-single">
      <div className="inspector-title">
        <div><p className="eyebrow">{t('structure.properties')}</p><h2>{selected?.name}</h2></div>
        <Badge tone={item?.freshness === 'FRESH' ? 'ok' : 'warning'}>{item?.freshness === 'FRESH' ? t('structure.current') : t('structure.stale')}</Badge>
      </div>
      <div className="inspector-resource-hero"><span className="resource-kind-icon large" aria-hidden="true">{selected?.type === 'CATEGORY' ? '▰' : selected?.type === 'THREAD' ? '↳' : '#'}</span><div><strong>{t(typeKey)}</strong><small>{t('structure.inspector.identity')}</small></div></div>
      <dl className="inspector-grid">
        <div><dt>{t('structure.inspector.discordId')}</dt><dd>{selected?.id}</dd></div>
        <div><dt>{t('structure.inspector.type')}</dt><dd>{t(typeKey)}</dd></div>
        <div><dt>{t('structure.inspector.parent')}</dt><dd>{item?.parent_id ?? t('structure.rootLabel')}</dd></div>
        <div><dt>{t('structure.inspector.position')}</dt><dd>{item?.position ?? selected?.position ?? '—'}</dd></div>
        <div><dt>{t('structure.inspector.freshness')}</dt><dd>{item?.freshness ?? 'UNKNOWN'}</dd></div>
        <div><dt>{t('structure.inspector.observability')}</dt><dd>{item?.observability ?? 'UNKNOWN'}</dd></div>
        <div><dt>{t('structure.inspector.assertion')}</dt><dd>{item?.data_assertion ?? 'UNKNOWN'}</dd></div>
        <div><dt>{t('structure.inspector.capability')}</dt><dd><Badge tone={structureWrite === 'CAN' ? 'ok' : structureWrite === 'CANNOT' ? 'danger' : 'warning'}>{structureWrite}</Badge></dd></div>
      </dl>
      <button type="button" className="inspector-primary-action" onClick={(event) => onActions(event.currentTarget)}>{t('structure.selection.actions')}</button>
    </div>
  )
...
function DropTargetCard({ destination, hoverKey, title, subtitle, icon }: { destination: ResourceRef; hoverKey: string | null; title: string; subtitle: string; icon: string }) {
  const key = dropKey(destination)
  return (
    <div
      className={`destination-drop-card ${hoverKey === key ? 'drop-hover' : ''}`}
      data-drop-id={destination.id}
      data-drop-guild={destination.guildId}
      data-drop-name={destination.name}
      data-drop-type={destination.type}
      data-drop-position={destination.position}
      data-drop-parent={destination.parentId ?? ''}
    >
      <span className="destination-icon" aria-hidden="true">{icon}</span>
...
function DestinationGuildCard({ guild, userId, hoverKey }: { guild: Guild; userId: DiscordSnowflake; hoverKey: string | null }) {
  ...
  const guildDestination: ResourceRef = { id: guild.guild_id, name: guild.name, type: 'GUILD', guildId: guild.guild_id }
  const [expanded, setExpanded] = useState(false)
  return (
    <section className={`destination-guild ${expanded ? 'expanded' : ''}`}>
      <div
        className={`destination-guild-row ${hoverKey === dropKey(guildDestination) ? 'drop-hover' : ''}`}
        data-drop-id={guild.guild_id}
        data-drop-guild={guild.guild_id}
        data-drop-name={guild.name}
        data-drop-type="GUILD"
      >
        <span className="destination-server-avatar">{guild.name.slice(0, 2).toUpperCase()}</span>
        <span className="destination-server-copy"><strong>{guild.name}</strong><small>{t('structure.drop.copy', { name: guild.name })}</small></span>
        <button type="button" aria-label={expanded ? t('structure.tree.collapse', { name: guild.name }) : t('structure.tree.expand', { name: guild.name })} onClick={() => setExpanded((value) => !value)}>{expanded ? '⌄' : '›'}</button>
      </div>
      {expanded && (
        <div className="destination-guild-tree">
          {structure.isLoading && <small>{t('structure.destinations.loading')}</small>}
          {structure.isError && <small>{t('structure.destinations.unavailable')}</small>}
          {structure.data?.categories.map((category) => {
            const destination = resourceRef(category)
            return <DropTargetCard key={category.id} destination={destination} hoverKey={hoverKey} title={category.name} subtitle={t('structure.drop.copy', { name: category.name })} icon="▰" />
          })}
        </div>
      )}
    </section>
