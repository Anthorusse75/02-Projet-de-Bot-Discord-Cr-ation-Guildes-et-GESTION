...
function fixture() {
  ...
  const group = (id: string, name: string) => ({ id, guild_id: guildId, name, root_kind: 'CHANNEL_SET', routing_mode: 'HUB_AND_SPOKE', visibility_scope_id: 'scope', source_language_profile_id: frId, provider_binding_id: name === 'Guides' ? 'provider-ready' : 'provider-manual', status: 'ACTIVE', version: 2, languages, category_variants: [], channel_groups: [{ id: `${id}-channel-group`, logical_key: `${name}-guides`, display_name: `${name} guides`, source_language_profile_id: frId }], channel_variants: [{ id: `${id}-variant`, language_profile_id: frId, discord_channel_id: discordSnowflake('700000000000000010'), state: name === 'Guides' ? 'MISSING' : 'ACTIVE', translation_channel_group_id: `${id}-channel-group`, translation_category_variant_id: null }], routes: [{ id: `${id}-route`, source_language_profile_id: frId, destination_language_profile_id: enId, state: 'ACTIVE' }] })
return { guild_id: guildId, source: 'DURABLE_TOPOLOGY_AND_LOCAL_DISCORD_CACHE' as const, discord_rest_calls: 0 as const, cache_coverage: { mode: 'FULL', freshness: 'FRESH', roles_complete: true, channels_complete: true, members_complete: true, state_version: 1 }, languages, groups: [group('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa','Guides'), group('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb','Support')], providers: [{ id: 'provider-ready', provider_type: 'existing_translation_bot', status: 'READY', capabilities_json: { supports_hub_and_spoke: true }, last_validated_at: '2026-08-30T00:00:00Z' }, { id: 'provider-manual', provider_type: 'existing_translation_bot', status: 'MANUAL_CONFIGURATION_REQUIRED', capabilities_json: {}, last_validated_at: null }], visibility_bindings: [{ id: 'binding', visibility_scope_id: 'scope', language_profile_id: frId, discord_role_id: discordSnowflake('700000000000000020'), state: 'ACTIVE' }], resource_language_policies: [{ id: 'policy', resource_type: 'CHANNEL' as const, discord_resource_id: discordSnowflake('700000000000000010'), explicit_language_profile_id: frId, inherit_language: false, visibility_policy: 'SCOPE_AND_LANGUAGE' as const, visibility_scope_id: 'scope' }] }
}
...
describe('STAGE 08 translation workspace', () => {
  ...
  it('keeps two FR/EN groups independent and exposes manual provider, drift, visibility and capacity', () => {
    ...
    expect(screen.getAllByText('Manual configuration required').length).toBeGreaterThan(0)
    ...
    const guides = screen.getByRole('heading', { name: 'Guides' }).closest<HTMLElement>('article')
    const support = screen.getByRole('heading', { name: 'Support' }).closest<HTMLElement>('article')
    if (!guides || !support) throw new Error('group cards missing')
    expect(within(guides).getByRole('button', { name: 'Create variant' })).toBeEnabled()
    expect(within(support).getByRole('button', { name: 'Create variant' })).toBeDisabled()
  })
