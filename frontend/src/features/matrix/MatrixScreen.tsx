...
export function MatrixScreen() {
  ...
  function openCell(cell: AccessMatrixCell, roleName: string, resource: MatrixResource) {
    const first = compatibleNativePolicies(resource.kind)[0]
  ...
  async function previewCell() {
    ...
    const intent = compatibleNativePolicies(activeCell.resource.kind).find((item) => item.id === cellIntentId)
    if (!intent) return
