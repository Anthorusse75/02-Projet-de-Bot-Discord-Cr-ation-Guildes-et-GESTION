...
import { phase4Overrides } from './phase4Overrides'
...
export function activationCandidates(override: LocaleChoice, languages: readonly string[], active: readonly string[]): LocaleCode[] {
  const preferred = override !== null && active.includes(override) ? override : null
  return [...new Set([preferred, resolveLocale(languages, active), 'en'].filter((value): value is LocaleCode => Boolean(value)))]
}
...
function bundledPack(locale: string): Record<string, string> | undefined {
  ...
  return { ...bootstrapPacks[code], ...phase2Packs[code], ...phase3Packs[code], ...phase4Packs[code], ...phase4PoliciesPacks[code], ...phase4WizardPacks[code], ...phase4MatrixPacks[code], ...phase4Overrides[code] }
}

const bundledResources = Object.fromEntries(
  bootstrap.map((code) => [code, { translation: { ...bootstrapPacks[code], ...phase2Packs[code], ...phase3Packs[code], ...phase4Packs[code], ...phase4PoliciesPacks[code], ...phase4WizardPacks[code], ...phase4MatrixPacks[code], ...phase4Overrides[code] } }]),
)
...
export function LocalizationProvider({ children }: { children: ReactNode }) {
  ...
  const [locale, setLocale] = useState<LocaleCode>('en'); const [activationGeneration, setActivationGeneration] = useState(0)
  ...
  useEffect(() => { let current = true; async function activate() { for (const candidate of activationCandidates(override, navigator.languages, activeCodes)) { const pack = bundledPack(candidate) ?? await runtimePack(candidate); if (!current) return; if (!pack) continue; i18n.addResourceBundle(candidate, 'translation', pack, true, true); await i18n.changeLanguage(candidate); if (!current) return; document.documentElement.lang = candidate; document.documentElement.dir = activeLocales.find((item) => item.locale_code === candidate)?.direction ?? 'ltr'; setLocale(candidate); return } } void activate(); return () => { current = false } }, [activeCodes.join('\0'), activationGeneration, override])
  useEffect(() => { const changed = () => setActivationGeneration((value) => value + 1); window.addEventListener('languagechange', changed); return () => window.removeEventListener('languagechange', changed) }, [])
  function hydrateServerPreference(value: LocaleChoice) { if (value === null || localePattern.test(value)) { if (value === null) localStorage.removeItem('did.uiLocaleOverride'); else localStorage.setItem('did.uiLocaleOverride', value); setOverrideState(value); setActivationGeneration((generation) => generation + 1) } }
