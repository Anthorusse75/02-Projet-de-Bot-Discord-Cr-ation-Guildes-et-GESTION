...
test('single-resource context menu prioritizes Manage access and preserves the exact scope', async ({ page }) => {
  ...
  await resourceRow(page, 'welcome').click({ button: 'right' })
  const mobileMenu = page.getByRole('menu', { name: 'Available actions' })
  const mobileBounds = await mobileMenu.boundingBox()
  expect(mobileBounds).not.toBeNull()
  expect((mobileBounds?.x ?? -1) + (mobileBounds?.width ?? 0)).toBeLessThanOrEqual(382)
  expect((mobileBounds?.y ?? -1) + (mobileBounds?.height ?? 0)).toBeLessThanOrEqual(836)
