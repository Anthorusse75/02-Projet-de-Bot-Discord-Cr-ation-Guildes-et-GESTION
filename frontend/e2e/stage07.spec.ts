...
test('an invalid active runtime catalog falls back atomically', async ({ page }) => {
  await mockDashboard(page); await page.route('**/api/v1/ui/locales', (route) => route.fulfill({ json: { locales: [{ locale_code: 'en', display_name: 'English', flag_code: 'gb', direction: 'ltr' }, { locale_code: 'it', display_name: 'Italiano', flag_code: 'it', direction: 'ltr' }] } })); await page.route('**/api/v1/ui/locales/it/catalog/*', (route) => route.fulfill({ json: { catalog_version: 'did-ui-v2', payload: { 'app.title': '<script>bad</script>' } } })); await page.route('**/api/v1/me/preferences', (route) => route.fulfill({ json: { ui_locale_override_code: 'it', timezone: null } }))
...
test('invalid persisted Italian runtime pack falls from previous French to complete browser German and can reactivate', async ({ page }) => {
  ...
  await page.route('**/api/v1/ui/locales', (route) => route.fulfill({ json: { catalog_version: 'did-ui-v2', locales: [{ locale_code: 'en', display_name: 'English', flag_code: 'gb', direction: 'ltr' }, { locale_code: 'fr', display_name: 'Français', flag_code: 'fr', direction: 'ltr' }, { locale_code: 'de', display_name: 'Deutsch', flag_code: 'de', direction: 'ltr' }, { locale_code: 'it', display_name: 'Italiano', flag_code: 'it', direction: 'ltr' }] } }))
  await page.route('**/api/v1/ui/locales/it/catalog/*', (route) => route.fulfill({ json: { catalog_version: 'did-ui-v2', payload: valid ? italian : { 'app.title': '<script>bad</script>' } } }))
