...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            name="fk_logical_groups_installation",
            ondelete="CASCADE",
        ),
    ...
    op.create_index(
        "uq_logical_group_resource_channel",
        "logical_group_resources",
        ["guild_id", "logical_group_id", "resource_type", "discord_channel_id"],
        unique=True,
        postgresql_where=sa.text("discord_channel_id IS NOT NULL"),
    )
    op.create_index(
        "uq_logical_group_resource_role",
        "logical_group_resources",
        ["guild_id", "logical_group_id", "resource_type", "discord_role_id"],
        unique=True,
        postgresql_where=sa.text("discord_role_id IS NOT NULL"),
    )
    op.create_table(
        "visibility_scopes",
        sa.Column("id", sa.Uuid(), nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ["guild_id"],
            ["guild_installations.guild_id"],
            name="fk_visibility_scopes_installation",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["guild_id", "logical_group_id"],
            ["logical_groups.guild_id", "logical_groups.id"],
            name="fk_visibility_scopes_logical_group",
            ondelete="RESTRICT",
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            name="fk_scope_membership_rules_scope",
            ondelete="CASCADE",
        ),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            "status IN ('ACTIVE','DISABLED')", name="ck_scope_membership_rules_status"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["guild_id", "visibility_scope_id"],
            ["visibility_scopes.guild_id", "visibility_scopes.id"],
            name="fk_scope_explicit_memberships_scope",
            ondelete="CASCADE",
        ),
