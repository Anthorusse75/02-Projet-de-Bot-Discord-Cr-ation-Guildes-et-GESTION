...
- Intention : l'idempotency de transfer identifie artifact/B/mode/caller key et permet de compléter le mapping jusqu'à READY. La transition vers READY fige atomiquement la résolution complète et un SHA-256 canonique `{explicit,resolved}` : logical ref, type, décision, cible et état de confirmation sont sémantiques ; scores, raisons, candidats, wording, timestamps et ordre ne le sont pas. Un retry READY revalide B et exige le même hash avant compilation ; cible disparue, candidat alternatif ou changement CREATE/MANUAL échoue avant tout plan. La clé de plan dépend du hash figé. COMPILED est un CAS depuis READY; plan, mapping et report deviennent immuables. Une reprise strictement identique retourne l'existant, toute divergence produit un conflit stable sans second plan. Les anciennes lignes READY/COMPILED sans matière sémantique complète sont invalidées fail-closed par 0012.

## IMP-016 — Traduction Discord des intentions Policy avancées

- Date : 2026-09-16
- Statut : `RESOLVED_OFFICIAL_CONTRACT_CONFIRMED`
- Sources officielles relues : [Permissions](https://docs.discord.com/developers/topics/permissions), [Channel Resource](https://docs.discord.com/developers/resources/channel) et [Threads](https://docs.discord.com/developers/topics/threads).
- Décision d’architecture : `ACCESS_CONTROL` reste la seule Policy et le `PolicyResolver` la seule résolution. La traduction intention → permissions est contextuelle et renvoyée avec la résolution ; le DSG/Plan consomme ces bitfields. Le frontend et les routers ne recalculent aucun droit.
- Vocal : rejoindre, parler et gérer restent trois intentions distinctes. Un Stage traduit la prise de parole par `REQUEST_TO_SPEAK`; la gestion demande seulement les flags applicables au type de channel.
- Threads : texte normal compile public + privé, announcement seulement public, forum/media utilise `SEND_MESSAGES`; participer utilise `SEND_MESSAGES_IN_THREADS`.
- Réactions et mentions : `ADD_REACTIONS` ne bloque que l’ajout initial d’une réaction ; réutiliser une réaction existante peut rester possible. `@everyone` et `@here` partagent `MENTION_EVERYONE`, tandis que la propriété mentionnable d’un rôle est globale. L’UI doit montrer ces limites et ne jamais prétendre séparer des capacités que Discord ne sépare pas.
- Bots : la sélection est bornée aux bots observés du read model. Les fonctions humaines utilisent les intentions canoniques granulaires `VIEW`, `READ_HISTORY`, `SEND` et `MANAGE_CHANNEL`, complétées par les intentions threads/vocal selon le contexte. Elles se traduisent exactement vers le minimum affiché et ne recommandent jamais `ADMINISTRATOR`; une identité, couverture ou fraîcheur insuffisante produit `UNKNOWN` avec remédiation.
