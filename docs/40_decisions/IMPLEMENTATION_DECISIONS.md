...
- Gateway/impact : les expected mutations bulk sont itemisées; celles des overwrites portent channel, target et état complet. Les matchers sont stricts et `plan_resource_dependencies` limite le stale aux plans concernés. L'impact réutilise le Permission Evaluator STAGE 04 et conserve l'inconnu lorsque la couverture membres est incomplète.
- Live : le runner a compilé et persisté un plan réel, puis le preflight a refusé toute mutation car le bot sandbox ne possède ni `MANAGE_CHANNELS` ni `MANAGE_ROLES`. Statut honnête : `BLOCKED_CAPABILITY_CONFIGURATION`, zéro mutation; crash-window et cleanup restent non vérifiés jusqu'à correction humaine minimale de la Guild sandbox B.
