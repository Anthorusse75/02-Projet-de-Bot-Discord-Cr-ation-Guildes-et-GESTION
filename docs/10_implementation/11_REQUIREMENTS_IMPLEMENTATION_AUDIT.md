...
Libellés canoniques : `00_REQUIREMENTS_TRACEABILITY.md` (246 historiques) et `10_EXIGENCES_DETAILLEES_ET_AUDIT_CONFORMITE.md` (143 détaillées).
...
- Permissions : `permissions/calculator.py`, `permissions/views.py`, tests unit/property; rôles cumulatifs, overwrites, owner/admin, View As, simple/expert, simulation.
- Planning : `application/planning/service.py`, `planning/models.py`, persistance/DAG/preflight/impact/UNKNOWN_OUTCOME, failure-injection.
- Portabilité/templates : `application/portability/service.py`, `portability_repository.py`, `api/stage06.py`, tests PostgreSQL/E2E; template privé RLS démontré.
...
1. **P0 Policy Engine générique** : modèle/version/lifecycle/scope, resolver, conflits/héritage, RLS/RBAC, explain/preview/preflight/plan/tests.
2. **P0 REQ-TEST-003** : restaurer les credentials sandbox et produire l’agrégat A/B du commit courant.
3. **P1 Wizards**, **Operations Center inter-session + drafts**, **catalogue de templates adaptatifs**.
4. **P2 UX** : second clic/F2/rename contextuel, label des groupes logiques, emoji picker/naming stylé; formaliser la doctrine REUSE.

## Maintenance

Une exigence `PARTIEL` ne passe à `CONFORME` qu’après satisfaction de **tous** ses critères avec preuve code/test/artefact. Mettre à jour uniquement les IDs affectés par chaque commit.
