...
> **Statut :** `STAGE_09_COMPLETE_DRAFT_PR_OPEN`. Trois passes de remédiation cumulées corrigent
> l'intégralité de la chaîne de traduction : (1) le défaut fail-open de l'adaptateur `googletrans`
> (accepte désormais fermé toute panne de transport) ; (2) une fuite de ressources dans la
> qualification live (nettoyage désormais garanti sous toute sortie) ; (3) **cette passe** — une vraie
> mutation intermittente de placeholder côté provider, root-causée et corrigée par une retry bornée
> avec régénération de placeholders, jamais un relâchement du fail-closed. **Nouvelle évidence réelle
> du product owner (réseau différent de ce sandbox) prouve que le provider `googletrans` est
> opérationnel** — la conclusion précédente d'indisponibilité globale était trop large, propre à ce
> sandbox réseau spécifique ; voir « État actuel » ci-dessous pour la distinction exacte. Seule une
> confirmation externe (le product owner exécutant le benchmark canonique depuis son réseau
> fonctionnel) reste ouverte. **PR #9 reste Draft, `DO NOT MERGE — EXTERNAL AUDIT REQUIRED`.**
...
| Suite de tests | `pytest backend/tests/unit` + `integration` (régression complète, non filtrée par stage) ; **cette passe** : 16 nouveaux tests déterministes (8 dans `test_stage09_rendering.py` prouvant la reprise bornée sur mutation de placeholder au niveau `render_field_text`/`render_message_model`, 9 dans le nouveau `test_stage09_translation_benchmark_production.py` prouvant la même logique côté script de benchmark) ; voir « Validation canonique » ci-dessous pour les décomptes PASS exacts |
| Qualification live Discord (primitives + chaîne complète) | **Évidence inchangée depuis la passe précédente, propre à CE sandbox réseau** (où le provider reste indisponible -- voir « Disponibilité réelle du provider » ci-dessus) : `python scripts/validate_stage.py 09 --include-discord-live` -- 9 des 11 groupes 100% PASS (rien lié à la traduction), `translation_group_provider_boundary` 100% PASS, `translation_group_did_fanout` échoue honnêtement sur ce qui dépend réellement d'une traduction obtenue. Cette passe n'a pas rejoué cette évidence (aucun accès réseau fonctionnel dans ce sandbox) -- la correction de mutation de placeholder (cette passe) n'a donc pas encore été vérifiée contre CE chemin live spécifique ; elle l'a été de façon déterministe (16 tests, voir ci-dessus) et par le product owner au niveau benchmark (voir ci-dessus) |
