...
> **Statut :** `STAGE_09_COMPLETE_DRAFT_PR_OPEN`. Onze passes de remédiation cumulées corrigent
...
> correctif avant toute régénération du pack de revue humaine.**
...
| Statut `CURRENT_STATE` | `STAGE_09_COMPLETE_DRAFT_PR_OPEN` (voir `docs/10_implementation/00_CURRENT_STATE.md`) — toutes les clauses techniques/canoniques sont satisfaites avec preuve réelle ou déterministe ; il reste deux confirmations externes ouvertes (product owner) : rejouer la qualification réseau ciblée de la Root cause 8/9 (ci-dessous, après audit externe du correctif Unicode) PUIS la régénération et le remplissage du pack de revue humaine — voir Root cause 9 et « Limitations honnêtement externes restantes » |
...
| **Pack de revue sémantique humaine (36 échantillons) -- généré une première fois cette passe, a révélé la Root cause 8, DOIT être régénéré après qualification réseau avant tout remplissage humain** | Script reproductible `scripts/generate_human_semantic_review_pack.py` -- 36 appels de traduction réels (12 paires dirigées × 3 classes identiques par langue : `negation_and_pronouns`/`long_sentence`/`mixed_technical_and_linguistic`), via le pipeline de production réel (`did.campaigns.rendering.render_field_text`, même provider RPC, même retry bornée), texte source et texte restauré **complets** (jamais tronqués). Le product owner l'a déjà exécuté une fois contre le SHA `92fa8aae18542416790767909e45a755ee6e321e` -- ce run a servi à DÉCOUVRIR la Root cause 8 (voir ci-dessus), pas à noter la qualité linguistique ; aucun score humain n'a été rempli, à dessein. Ce pack PRÉ-correctif est préservé tel quel comme évidence historique négative à `docs/90_handoffs/evidence/stage09/human-semantic-review-pre-boundary-fix-92fa8aae.md` -- jamais réécrit. Commande exacte pour RÉGÉNÉRER le pack une fois la qualification réseau ci-dessus confirmée 12/12 : `uv run python scripts/generate_human_semantic_review_pack.py --sha <SHA post-qualification>` (écrit `docs/90_handoffs/evidence/stage09/HUMAN_SEMANTIC_REVIEW.md`). Tous les champs de jugement humain (fidélité sémantique, naturel, terminologie/contexte, mistraduction bloquante, commentaires, relecteur, date, PASS/FAIL humain) restent **intentionnellement vides** -- jamais un score assigné par l'agent d'implémentation ou par un outil IA, ce qui inclut explicitement Claude/Codex. Statut : `PENDING_HUMAN_REVIEW` jusqu'à ce qu'un relecteur humain compétent dans la langue cible jugée remplisse réellement ces champs -- jamais l'inverse, et jamais AVANT la qualification réseau ci-dessus. 23 tests déterministes (offline, traducteur factice injecté) prouvent la mécanique du générateur lui-même (36 lignes, 12 directions, 3 items chacune, jamais de paire même-langue, texte complet, métadonnées d'intégrité techniques présentes, champs humains vides par construction) -- jamais une preuve de qualité linguistique. |
| Limitations honnêtement externes restantes | (A) **qualification réseau ciblée `mixed_technical_and_linguistic` (Root cause 8/9)** : exécutée une première fois cette passe -- **11/12 PASS, 1/12 FAIL** (EN→FR, défaut de reconnaissance Unicode maintenant corrigé, Root cause 9) -- **DOIT être REJOUÉE par le product owner contre le SHA contenant le correctif Unicode, après un nouvel audit externe** ; **NOUVEAU gate bloquant avant la revue humaine, toujours ouvert** ; (B) **revue sémantique humaine** : `PENDING_HUMAN_REVIEW`, mais NE DOIT PAS être régénérée/remplie avant que (A) confirme 12/12 avec un espacement correct -- les packs déjà générés (contre le SHA `92fa8aae...`) sont des évidences historiques de défaut, jamais des packs prêts à noter ; (C) **qualification réseau/benchmark d'intégrité de placeholders** : **CLOSE** -- smoke réel et benchmark canonique complet tous deux `PASS` contre le SHA `f19d6616a170386995664de4f92fd65347063ba0`, aucune commande réseau supplémentaire n'est requise POUR CE SUJET SPÉCIFIQUE (l'intégrité du multi-ensemble de placeholders, distincte de la présentation/espacement couverte par (A)) ; (D) participation live d'un provider de traduction tiers réel attaché à un Translation Group : `EXTERNAL_SANDBOX_CAPABILITY_NOT_AVAILABLE`, inchangé ; (E) reproduction sûre d'un `UNKNOWN_OUTCOME` réel contre Discord : `NOT_SAFELY_REPRODUCIBLE_LIVE`, inchangé |
