...
> **Statut :** `STAGE_09_COMPLETE_DRAFT_PR_OPEN`. Cinq passes de remédiation cumulées corrigent
...
> réseau dès qu'une reprise ou un échec partiel survenait), corrigé par un compteur explicite ; (5)
> **cette passe** — le transport `googletrans` `/translate_a/single` lui-même s'est révélé
> **spécifiquement** indisponible (HTTP 429, prouvé sur plusieurs machines du réseau du product owner),
> tandis qu'une sonde indépendante de l'autre endpoint web non officiel de Google (le RPC
> `batchexecute` que le site translate.google.com utilise lui-même) a réussi (HTTP 200, vraie
> traduction) depuis la même machine. **Le transport de production Stage09 est donc changé** vers ce
> RPC (`GoogleTranslateRpcCampaignTranslationProvider`, nouveau module `did.translation
> .google_translate_rpc_adapter`) -- `googletrans` n'est plus jamais construit sur aucun chemin
> production/live/benchmark, mais reste dans le dépôt comme code historique/comparatif. Toutes les
> garanties fail-closed (aucun fallback source non traduite, aucune chaîne vide, aucun switch de
> provider silencieux) sont préservées à l'identique sur le nouveau transport. Seule une confirmation
> externe (le product owner exécutant le smoke réseau réel puis le benchmark canonique depuis son
> réseau fonctionnel) reste ouverte. **PR #9 reste Draft, `DO NOT MERGE — EXTERNAL AUDIT REQUIRED`.**
...
| Suite de tests | `pytest backend/tests/unit` + `integration` (régression complète, non filtrée par stage) ; **cette passe** : 38 nouveaux tests déterministes dans le nouveau `test_stage09_google_translate_rpc_adapter.py` (parsing RPC valide/malformé, Unicode FR/EN/DE/ES, HTTP 403/429/5xx, timeout, erreur de transport générique, intégration circuit breaker, cycle de vie exact du client HTTP, et intégration bout-en-bout prouvant qu'une mutation de placeholder produite par CET adaptateur atteint bien le validateur existant et est rejetée/récupérée par la retry bornée) -- aucun réseau requis, `httpx.MockTransport` simule le transport HTTP. Cumulé avec les 32 tests des passes précédentes (`test_stage09_rendering.py` + `test_stage09_translation_benchmark_production.py`, inchangés), voir « Validation canonique » ci-dessous pour les décomptes PASS exacts |
...
| Smoke réseau réel étendu -- évidence committée (ce sandbox) | `backend/tests/network/test_stage09_translation_network.py` -- réécrit cette passe pour exercer le nouveau provider de production (`GoogleTranslateRpcCampaignTranslationProvider`, transport RPC) au lieu de l'ancien `GoogletransCampaignTranslationProvider`, mais **pas exécuté depuis ce sandbox** (aucun accès réseau fonctionnel ici, et la mission interdit explicitement toute dépense réseau significative depuis cet environnement). Aucun résultat committé par cette passe -- c'est exactement la commande que le product owner doit exécuter en premier ci-dessous, avant tout benchmark |
| **Commande de confirmation finale (item 15, en DEUX étapes séquentielles, à exécuter par le product owner)** | **Étape 1 -- petit smoke réseau réel d'abord** : `uv run pytest backend/tests/network/test_stage09_translation_network.py -m translation_network` (ou l'équivalent `DID_ALLOW_NETWORK=1`) depuis le réseau où la sonde RPC a déjà réussi. Ce test exerce maintenant directement `GoogleTranslateRpcCampaignTranslationProvider`, la même construction que `did.runtime.py` -- doit passer 5/5 avant toute autre étape. **Étape 2 -- seulement si l'étape 1 passe** : `python scripts/validate_stage.py 09 --profile translation-benchmark --allow-network`. Doit rapporter `PASS` sur le bucket `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` (jamais sur le bucket `FULL_MASKED_MESSAGE` brut, qui reste une mesure comparative sans retry) -- c'est la SEULE évidence encore manquante avant que `STAGE_09_COMPLETE_DRAFT_PR_OPEN` puisse être considéré pleinement confirmé de bout en bout sur le nouveau transport. Le comptage `provider_invocations`/`provider_invocations_all_strategies` du rapport JSON produit par cette commande reste exact (corrigé passe précédente, revérifié inchangé cette passe -- voir Root cause 3 ci-dessus). **Ne PAS lancer l'étape 2 avant que l'étape 1 confirme le transport RPC fonctionnel** -- c'est exactement l'erreur qui a coûté un run de ~35 minutes la passe précédente avec l'ancien transport. |
