...
> **Statut :** `STAGE_09_COMPLETE_DRAFT_PR_OPEN`. Neuf passes de remédiation cumulées corrigent
...
> dépôt depuis ce sandbox, `artifacts/` étant gitignored par conception).
...
| Suite de tests | `pytest backend/tests/unit` + `integration` (régression complète, non filtrée par stage) ; passes précédentes : `test_stage09_google_translate_rpc_adapter.py` réécrit contre le contrat de wire corrigé (Root cause 5), `test_stage09_translation_benchmark_production.py`'s `TestTransportAttemptAccounting` (Root cause 6), `test_stage09_parser_protector.py`'s `TestUrlBoundarySpacingSourceProvenRepair` (Root cause 7, 13 tests) et `test_stage09_rendering.py`'s `TestUrlBoundarySpacingDoesNotTriggerAnArtificialRetry` (1 test). **Cette passe** : nouveau `test_stage09_human_semantic_review_pack.py` (23 tests déterministes, offline, traducteur factice injecté -- jamais un vrai appel réseau) pour le nouveau script `scripts/generate_human_semantic_review_pack.py`, prouvant : exactement 36 lignes, exactement 12 paires dirigées à 3 items chacune, jamais de paire même-langue, texte source/restauré complet jamais tronqué, métadonnées d'intégrité technique présentes, champs de jugement humain vides par construction, aucun contenu hors du corpus synthétique committé, sortie déterministe hors résultat de traduction/horodatage runtime. Toujours aucun réseau requis dans la suite déterministe, `httpx.MockTransport` simule le transport HTTP là où pertinent ; voir « Validation canonique » ci-dessous pour les décomptes PASS exacts |
...
| **Commande de confirmation finale -- TOUTES LES ÉTAPES CONFIRMÉES** | **Étape 1** : `5 passed in 6.16s` (confirmé passe -2). **Étape 2** (`python scripts/validate_stage.py 09 --profile translation-benchmark --allow-network`) : **`PASS`** (confirmé cette passe, voir ligne « évidence CANONIQUE COURANTE » ci-dessus). **Il n'existe donc plus de commande de qualification réseau bloquante pour Stage09** -- ne PAS rejouer ce benchmark à nouveau (~2272 appels) : cette évidence PASS reste valide et ne doit pas être régénérée inutilement. Le seul écart d'évidence externe restant est désormais purement HUMAIN, voir la ligne « Pack de revue sémantique humaine » ci-dessous. |
| **Pack de revue sémantique humaine (36 échantillons) -- nouveau cette passe, reste à exécuter puis à faire remplir par un humain** | Script reproductible `scripts/generate_human_semantic_review_pack.py` -- 36 appels de traduction réels (12 paires dirigées × 3 classes identiques par langue : `negation_and_pronouns`/`long_sentence`/`mixed_technical_and_linguistic`), via le pipeline de production réel INCHANGÉ (`did.campaigns.rendering.render_field_text`, même provider RPC, même retry bornée), texte source et texte restauré **complets** (jamais tronqués). Commande exacte pour le product owner : `uv run python scripts/generate_human_semantic_review_pack.py --sha f19d6616a170386995664de4f92fd65347063ba0` (écrit `docs/90_handoffs/evidence/stage09/HUMAN_SEMANTIC_REVIEW.md`). Tous les champs de jugement humain (fidélité sémantique, naturel, terminologie/contexte, mistraduction bloquante, commentaires, relecteur, date, PASS/FAIL humain) sont **intentionnellement vides** -- jamais un score assigné par l'agent d'implémentation ou par un outil IA, ce qui inclut explicitement Claude/Codex. Statut : `PENDING_HUMAN_REVIEW` jusqu'à ce qu'un relecteur humain compétent dans la langue cible jugée remplisse réellement ces champs -- jamais l'inverse. 23 nouveaux tests déterministes (offline, traducteur factice injecté) prouvent la mécanique du générateur (36 lignes, 12 directions, 3 items chacune, jamais de paire même-langue, texte complet, métadonnées d'intégrité techniques présentes, champs humains vides par construction) -- jamais une preuve de qualité linguistique. |
| Limitations honnêtement externes restantes | (A) **revue sémantique humaine** : `PENDING_HUMAN_REVIEW` -- le SEUL écart d'évidence externe restant ; le pack de 36 échantillons ci-dessus doit être généré pour de vrai par le product owner puis rempli par un relecteur humain compétent dans la langue cible jugée ; (B) **qualification réseau/benchmark** : **CLOSE** -- smoke réel et benchmark canonique complet tous deux `PASS` contre le SHA `f19d6616a170386995664de4f92fd65347063ba0`, aucune commande réseau supplémentaire n'est requise ; (C) participation live d'un provider de traduction tiers réel attaché à un Translation Group : `EXTERNAL_SANDBOX_CAPABILITY_NOT_AVAILABLE`, inchangé ; (D) reproduction sûre d'un `UNKNOWN_OUTCOME` réel contre Discord : `NOT_SAFELY_REPRODUCIBLE_LIVE`, inchangé |
