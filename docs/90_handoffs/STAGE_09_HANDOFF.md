...
> **Statut :** `STAGE_09_COMPLETE_DRAFT_PR_OPEN`. Sept passes de remédiation cumulées corrigent
...
> simples appels au port haut niveau. Seule une confirmation externe (le product owner exécutant le
> benchmark canonique avec ce comptage désormais réellement exact au niveau transport) reste ouverte.
...
| Suite de tests | `pytest backend/tests/unit` + `integration` (régression complète, non filtrée par stage) ; passe précédente : `test_stage09_google_translate_rpc_adapter.py` intégralement réécrit contre le contrat de wire corrigé (voir Root cause 5) -- fixture canonique reflétant la forme réellement capturée, frontières fail-closed testées individuellement, `TestExactRequestContract` asserte l'enveloppe `f.req`/paramètres/`Referer` exacts. **Cette passe** : `test_stage09_translation_benchmark_production.py`'s `TestProviderInvocationAccounting` (basée sur un `_FakeProvider` qui ne peut pas prouver un comptage réseau) remplacée par `TestTransportAttemptAccounting` (10 tests, pilote le vrai adaptateur contre `httpx.MockTransport`) -- voir Root cause 6 ci-dessus. `test_stage09_rendering.py` inchangé. Toujours aucun réseau requis dans cette suite, `httpx.MockTransport` simule le transport HTTP ; voir « Validation canonique » ci-dessous pour les décomptes PASS exacts |
...
| **Smoke réseau réel étendu -- ÉTAPE 1 CONFIRMÉE par le product owner** | `backend/tests/network/test_stage09_translation_network.py` exerce directement `GoogleTranslateRpcCampaignTranslationProvider` avec le contrat de wire corrigé (Root cause 5). **Exécuté réellement par le product owner depuis le Zenbook** contre le SHA `d206aa46117bdcfc64b0f60404ba524e7dc53e51` : `DID_ALLOW_NETWORK=1 uv run pytest backend/tests/network/test_stage09_translation_network.py -m translation_network` → **`5 passed in 6.16s`**. Le contrat de wire RPC corrigé est donc désormais confirmé empiriquement contre le vrai endpoint, pas seulement par cohérence interne du parseur. Toujours pas exécuté depuis ce sandbox (aucun accès réseau fonctionnel ici) ; aucun résultat committé en JSON par cette passe-ci (l'évidence ci-dessus est rapportée textuellement avec attribution explicite, comme pour toute évidence produite hors de ce sandbox). |
| **Commande de confirmation finale (item 15, en DEUX étapes séquentielles, à exécuter par le product owner)** | **Étape 1 -- CONFIRMÉE** (voir ligne ci-dessus, `5 passed in 6.16s`). **Étape 2 -- reste à exécuter** : `python scripts/validate_stage.py 09 --profile translation-benchmark --allow-network`. Doit rapporter `PASS` sur le bucket `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` (jamais sur le bucket `FULL_MASKED_MESSAGE` brut, qui reste une mesure comparative sans retry) -- c'est désormais la SEULE évidence encore manquante avant que `STAGE_09_COMPLETE_DRAFT_PR_OPEN` puisse être considéré pleinement confirmé de bout en bout. Le comptage `provider_invocations`/`provider_invocations_all_strategies` du rapport JSON produit par cette commande est désormais exact au niveau TRANSPORT, pas seulement au niveau du port `provider.translate(...)` -- voir Root cause 6 ci-dessus (deuxième correction de comptage, cette passe) ; il n'y avait donc aucune raison de faire rejouer ce benchmark une nouvelle fois au product owner avant cette correction, exactement comme pour Root cause 3 avant elle. |
