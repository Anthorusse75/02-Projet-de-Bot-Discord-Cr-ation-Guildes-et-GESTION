...
> **Statut :** `STAGE_09_COMPLETE_DRAFT_PR_OPEN`. Huit passes de remédiation cumulées corrigent
...
> Seule une confirmation externe (le product owner exécutant une qualification réseau réelle CIBLÉE sur
> les 12 directions `url_adversarial` -- voir « Commande de confirmation finale » ci-dessous -- PUIS,
> seulement si elle passe, le benchmark canonique complet) reste ouverte.
...
| Suite de tests | `pytest backend/tests/unit` + `integration` (régression complète, non filtrée par stage) ; passes précédentes : `test_stage09_google_translate_rpc_adapter.py` réécrit contre le contrat de wire corrigé (Root cause 5), `test_stage09_translation_benchmark_production.py`'s `TestTransportAttemptAccounting` (Root cause 6). **Cette passe** : `backend/src/did/messaging/protector.py` gagne `restore_source_proven_url_boundary_spacing()` (voir Root cause 7 ci-dessus) ; `test_stage09_parser_protector.py` gagne la classe `TestUrlBoundarySpacingSourceProvenRepair` (13 tests : reproduction forensique réelle end-to-end, non-modification quand déjà correct, preuve source obligatoire, portée URL-only, type de placeholder non-URL jamais touché, hallucination et mutation d'URL genuinement rejetées, intégrité de placeholder jamais contournable, deux URLs -- une seule frontière réparée) ; `test_stage09_rendering.py` gagne `TestUrlBoundarySpacingDoesNotTriggerAnArtificialRetry` (1 test, prouve qu'aucune reprise d'intégrité n'est plus nécessaire pour cette forme réelle). Nouveau fichier de qualification réseau ciblé (network-gated, non exécuté dans ce sandbox) : `test_stage09_translation_network_url_adversarial.py`. Toujours aucun réseau requis dans la suite déterministe, `httpx.MockTransport` simule le transport HTTP là où pertinent ; voir « Validation canonique » ci-dessous pour les décomptes PASS exacts |
...
| Benchmark de traduction -- évidence committée (ce sandbox) | `docs/90_handoffs/evidence/stage09/translation-benchmark.json` -- **inchangé cette passe, préservé tel quel comme évidence historique** : `BLOCKED`, 1248/1248 mesures en échec réel depuis CE sandbox, généré contre l'ancien transport `googletrans` `/translate_a/single` (provider indisponible ici -- HTTP 429/403, voir Root cause 4 ci-dessus). Cette passe n'a **pas** rejoué ni fabriqué de nouvelle évidence JSON de benchmark depuis ce sandbox -- aucun accès réseau fonctionnel disponible ici pour le faire honnêtement, et la mission interdit explicitement de dépenser des milliers d'appels réseau depuis cet environnement |
| **Benchmark de traduction canonique RPC -- évidence réelle du product owner, cette passe (rapportée, préservée, jamais fabriquée)** | **Non committée en JSON par cette passe** (rapportée textuellement avec attribution explicite, comme toute évidence produite hors de ce sandbox) : le product owner a exécuté le vrai benchmark canonique contre le SHA `1d71164f5ab24f1585048b3fcc226461d5b2ce1d`, comptage transport-exact (Root cause 6). Résultat : **`FAIL`, `provider_invocations_all_strategies = 2285`**. Par bucket : `FULL_MASKED_MESSAGE` 312 enregistrements/313 tentatives HTTP/0 erreur provider/300 sur 312 intégrité (0.9615) ; `PARAGRAPH_GROUPING` 312/337/0/300 sur 312 (0.9615) ; `SENTENCE_GROUPING` 312/535/1 erreur/310 sur 311 (0.9968, la seule erreur est une erreur de transport isolée, pas liée à ce défaut) ; `NAIVE_PER_TEXT_NODE` 312/775/5 erreurs/0.8827 (contrôle négatif attendu, jamais sélectionné par production) ; `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` 312/325/0 erreur/13 mesures retried/13 reprises d'intégrité totales/300 sur 312 (0.9615). **12/12 échecs de production isolés à la classe `url_adversarial`, sur les 12 paires de langues dirigées, chaque échec persistant après la seconde tentative** -- voir Root cause 7 ci-dessus pour la cause racine forensique exacte et le correctif. Cette évidence FAIL est délibérément préservée telle quelle, jamais réécrite en PASS -- c'est elle qui a motivé et guidé cette remédiation. |
| **Smoke réseau réel étendu -- ÉTAPE 1 CONFIRMÉE par le product owner (passe précédente)** | `backend/tests/network/test_stage09_translation_network.py` exerce directement `GoogleTranslateRpcCampaignTranslationProvider` avec le contrat de wire corrigé (Root cause 5). **Exécuté réellement par le product owner depuis le Zenbook** contre le SHA `d206aa46117bdcfc64b0f60404ba524e7dc53e51` : `DID_ALLOW_NETWORK=1 uv run pytest backend/tests/network/test_stage09_translation_network.py -m translation_network` → **`5 passed in 6.16s`**. Toujours pas exécuté depuis ce sandbox (aucun accès réseau fonctionnel ici). |
| **Qualification réseau ciblée `url_adversarial` -- ÉTAPE 1.5, nouvelle cette passe, reste à exécuter** | `backend/tests/network/test_stage09_translation_network_url_adversarial.py` -- 12 mesures réelles (une par paire de langues dirigée), réutilisant directement `_run_one_production()` du script de benchmark (donc la mesure de production exacte : adaptateur RPC réel, `FULL_MASKED_MESSAGE`, reprise d'intégrité bornée, `validate_full_pipeline()`), exclusivement sur la classe `url_adversarial`. Network-gated, non exécuté dans ce sandbox. Doit passer 12/12 (0 erreur provider, intégrité PASS pour chaque direction) avant tout nouveau benchmark complet -- voir « Commande de confirmation finale » ci-dessous. |
| **Commande de confirmation finale (en TROIS étapes séquentielles, à exécuter par le product owner)** | **Étape 1 -- CONFIRMÉE** (voir ligne ci-dessus, `5 passed in 6.16s`). **Étape 1.5 -- NOUVELLE cette passe, reste à exécuter** : `DID_ALLOW_NETWORK=1 uv run pytest backend/tests/network/test_stage09_translation_network_url_adversarial.py -m translation_network -v -s` -- doit passer 12/12 avant l'étape 2 ; c'est la première vérification empirique du correctif Root cause 7 contre le vrai endpoint. **Étape 2 -- seulement si l'étape 1.5 passe** : `python scripts/validate_stage.py 09 --profile translation-benchmark --allow-network`. Doit désormais rapporter `PASS` sur le bucket `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` (le run précédent, ci-dessus, a rapporté `FAIL` avant ce correctif) -- c'est désormais la SEULE évidence encore manquante avant que `STAGE_09_COMPLETE_DRAFT_PR_OPEN` puisse être considéré pleinement confirmé de bout en bout. Le comptage `provider_invocations`/`provider_invocations_all_strategies` du rapport JSON reste exact au niveau TRANSPORT (Root cause 6, inchangé cette passe). **Ne PAS lancer l'étape 2 avant que l'étape 1.5 confirme le correctif `url_adversarial`** -- c'est exactement pourquoi l'étape 1.5 existe : éviter de faire rejouer au product owner le benchmark complet (~2285 appels) une nouvelle fois avant d'avoir la preuve ciblée, peu coûteuse, que le correctif fonctionne réellement contre l'endpoint réel. |
| Limitations honnêtement externes restantes | (A) revue sémantique humaine : `PENDING_HUMAN_REVIEW`, inchangé ; (B) **confirmation de la qualification ciblée `url_adversarial` PUIS du benchmark canonique contre le transport RPC depuis un réseau fonctionnel** (clause mise à jour cette passe -- remplace l'ancienne clause qui considérait le comptage comme la seule évidence manquante) : le product owner doit exécuter les trois commandes ci-dessus, dans l'ordre, et committer son évidence JSON réelle ; (C) participation live d'un provider de traduction tiers réel attaché à un Translation Group : `EXTERNAL_SANDBOX_CAPABILITY_NOT_AVAILABLE`, inchangé ; (D) reproduction sûre d'un `UNKNOWN_OUTCOME` réel contre Discord : `NOT_SAFELY_REPRODUCIBLE_LIVE`, inchangé ; (E) le vrai benchmark canonique rapporté ci-dessus (2285 appels, `FAIL`) est la qualification statistique complète contre l'ancien code -- ce qui reste ouvert est la RECONFIRMATION contre le code corrigé de cette passe, d'abord ciblée (étape 1.5) puis complète (étape 2) |
