...
> **Statut :** `STAGE_09_COMPLETE_DRAFT_PR_OPEN`. Quatre passes de remédiation cumulées corrigent
...
> bornée avec régénération de placeholders, jamais un relâchement du fail-closed ; (4) **cette passe**
> — un défaut de comptage dans l'évidence du benchmark lui-même (`provider_invocations` sous-comptait
> les vrais appels réseau dès qu'une reprise ou un échec partiel survenait), corrigé par un compteur
> explicite. **Évidence réelle du product owner (réseau différent de ce sandbox) prouve que le
> provider `googletrans` est opérationnel** — la conclusion précédente d'indisponibilité globale était
> trop large, propre à ce sandbox réseau spécifique ; voir « État actuel » ci-dessous pour la
> distinction exacte. Seule une confirmation externe (le product owner exécutant le benchmark
> canonique corrigé depuis son réseau fonctionnel) reste ouverte. **PR #9 reste Draft,
> `DO NOT MERGE — EXTERNAL AUDIT REQUIRED`.**
...
| **Disponibilité réelle du provider (nuancée, item J)** | (a) Depuis l'environnement/IP de ce sandbox : le provider reste **indisponible** (HTTP 429/403 réels, prouvés, stables sur plusieurs tentatives). (b) Depuis l'environnement réseau du product owner : le provider est **opérationnel** -- 4 exécutions consécutives du smoke réseau réel à 5/5 PASS, puis le benchmark canonique complet à 1950 appels réels/0 erreur provider. La conclusion précédente (« provider globalement indisponible ») était donc trop large -- c'était une indisponibilité propre à CE sandbox réseau, pas une propriété du provider lui-même. (c) Le seul blocage restant est désormais une **qualification** : confirmer que la retry bornée AVEC comptage corrigé fonctionne contre le réseau réel du product owner -- voir la commande exacte en fin de section. |
| Runtime | `did.runtime.py` exécute le Campaign Engine réel (scheduler + worker + réconciliation de livraison, `GoogletransCampaignTranslationProvider` réel câblé, maintenant fail-closed + retry bornée sur mutation de placeholder) dans les process production |
| API HTTP / UI (Campaign Center) | Inchangées ces passes -- surface Stage09 complète (WP14), toutes les surfaces d'authoring produit |
| Suite de tests | `pytest backend/tests/unit` + `integration` (régression complète, non filtrée par stage) ; **cumul des deux dernières passes** : **32 nouveaux tests déterministes** -- `test_stage09_rendering.py` (12 : 8 sur la reprise bornée elle-même + 4 sur la validation `max_integrity_attempts >= 1`) et `test_stage09_translation_benchmark_production.py` (20 : 9 sur la logique de reprise côté benchmark + 11 sur le comptage exact des appels provider réels) ; voir « Validation canonique » ci-dessous pour les décomptes PASS exacts |
| Qualification live Discord (primitives + chaîne complète) | **Évidence inchangée depuis plusieurs passes, propre à CE sandbox réseau** (où le provider reste indisponible -- voir « Disponibilité réelle du provider » ci-dessus) : `python scripts/validate_stage.py 09 --include-discord-live` -- 9 des 11 groupes 100% PASS (rien lié à la traduction), `translation_group_provider_boundary` 100% PASS, `translation_group_did_fanout` échoue honnêtement sur ce qui dépend réellement d'une traduction obtenue. Ni la correction de mutation de placeholder ni celle du comptage n'ont donc encore été vérifiées contre CE chemin live spécifique dans ce sandbox -- elles l'ont été de façon déterministe (32 tests, voir ci-dessus) et par le product owner au niveau benchmark (voir ci-dessus) |
...
| Benchmark de traduction -- évidence committée (ce sandbox) | `docs/90_handoffs/evidence/stage09/translation-benchmark.json` -- **inchangé cette passe** : `BLOCKED`, 1248/1248 mesures en échec réel depuis CE sandbox (provider indisponible ici, voir ci-dessus). Cette passe n'a **pas** rejoué ni fabriqué de nouvelle évidence JSON depuis ce sandbox -- aucun accès réseau fonctionnel disponible ici pour le faire honnêtement |
| Benchmark de traduction -- évidence réelle du product owner (rapportée, item I : préservée, pas effacée) | **Non committée en JSON par cette passe** (le mission interdit de fabriquer/substituer une évidence que cette passe n'a pas elle-même produite) -- rapportée textuellement ici avec attribution explicite : 1950 appels réels, 0 erreur provider sur les 4 stratégies ; `FULL_MASKED_MESSAGE` 311/312 (99.68%, 1 échec isolé `es-inline-code-and-block-mixed` ES→FR) ; `PARAGRAPH_GROUPING` 312/312 (100%) ; `SENTENCE_GROUPING` 312/312 (100%) ; `NAIVE_PER_TEXT_NODE` 92.3% (contrôle négatif attendu). C'est cette évidence empirique qui a motivé la remédiation ci-dessus -- voir « Commande de confirmation finale » ci-dessous pour ce que le product owner doit maintenant rejouer avec le code corrigé |
| Smoke réseau réel étendu -- évidence committée (ce sandbox) | `backend/tests/network/test_stage09_translation_network.py` -- **inchangé cette passe**, toujours en échec réel depuis ce sandbox (429/circuit ouvert). Le product owner a rapporté 4 exécutions consécutives à 5/5 PASS depuis son réseau -- non committée en JSON par cette passe pour la même raison que ci-dessus |
| **Commande de confirmation finale (item L, à exécuter par le product owner)** | `python scripts/validate_stage.py 09 --profile translation-benchmark --allow-network` depuis un réseau où `googletrans` est confirmé opérationnel. Doit rapporter `PASS` sur le nouveau bucket `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` (jamais sur le bucket `FULL_MASKED_MESSAGE` brut, qui reste une mesure comparative sans retry) -- c'est la SEULE évidence encore manquante avant `STAGE_09_COMPLETE_DRAFT_PR_OPEN` puisse être considéré pleinement confirmé de bout en bout. **Corrigé avant de demander au product owner de lancer ce run (~35 minutes) une seconde fois** : le défaut de comptage `provider_invocations` ci-dessus est réparé, donc le champ `provider_invocations`/`provider_invocations_all_strategies` du rapport JSON produit par cette commande sera désormais exact -- il n'y avait aucune raison de faire rejouer le benchmark au product owner avant cette correction. |
| Limitations honnêtement externes restantes | (A) revue sémantique humaine : `PENDING_HUMAN_REVIEW`, inchangé ; (B) **confirmation du benchmark canonique avec retry depuis un réseau fonctionnel** (nouvelle clause cette passe, remplace l'ancienne « provider indisponible ») : le product owner doit exécuter la commande ci-dessus et committer son évidence JSON réelle ; (C) participation live d'un provider de traduction tiers réel attaché à un Translation Group : `EXTERNAL_SANDBOX_CAPABILITY_NOT_AVAILABLE`, inchangé ; (D) reproduction sûre d'un `UNKNOWN_OUTCOME` réel contre Discord : `NOT_SAFELY_REPRODUCIBLE_LIVE`, inchangé |
