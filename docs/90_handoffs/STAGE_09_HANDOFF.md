...
> **Statut :** `STAGE_09_COMPLETE_DRAFT_PR_OPEN`. Treize passes de remédiation cumulées corrigent
...
> aucun switch de provider silencieux) sont préservées à l'identique. **Le product owner a depuis
> exécuté ce smoke réseau réel depuis le Zenbook contre le contrat corrigé (SHA
> `d206aa46117bdcfc64b0f60404ba524e7dc53e51`) : `5 passed in 6.16s`** -- le contrat de wire RPC corrigé
> est donc désormais confirmé empiriquement, pas seulement par cohérence interne du parseur ; (7)
> **cette passe** — un second audit externe a trouvé que le compteur `provider_invocation_count`
> introduit en (4), bien qu'exact au niveau du port `provider.translate(...)`, sous-comptait quand même
> les vrais appels réseau : `GoogleTranslateRpcCampaignTranslationProvider.translate()` a sa PROPRE
> reprise bornée interne (`max_attempts=3`), donc un seul appel `translate()` peut générer jusqu'à 3
> vrais POST HTTP avant de réussir ou d'échouer -- le test déterministe HTTP-429-puis-succès de
> l'adaptateur le prouvait déjà. **Corrigé** : un nouveau mécanisme d'observation contextuel
> (`count_transport_attempts()`, `contextvars.ContextVar`, jamais un attribut mutable partagé sur
> l'instance du provider -- correct même si le benchmark devient un jour concurrent) notifié
> immédiatement avant chaque vrai `await client.post(...)` dans l'adaptateur ; le benchmark enveloppe
> désormais chaque mesure complète (tous les segments, toutes les reprises internes de l'adaptateur,
> toutes les tentatives de reprise d'intégrité) dans ce contexte, et dérive `provider_invocation_count`
> du delta réel observé, jamais plus d'un simple comptage d'appels à `provider.translate(...)`. Le champ
> JSON reste nommé `provider_invocation_count`/`provider_invocations` (aucune migration de schéma
> nécessaire) mais désigne désormais honnêtement le nombre exact de vraies tentatives HTTP RPC, pas de
> simples appels au port haut niveau. (8) **cette passe** — le product owner a exécuté le vrai benchmark
> canonique contre le SHA `1d71164f5ab24f1585048b3fcc226461d5b2ce1d` (comptage désormais exact au niveau
> transport) : **2285 vraies tentatives HTTP RPC, `FAIL`**, `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY`
> à 300/312 (96.15%). Isolation exacte : **12/12 échecs de production = classe `url_adversarial`, sur
> les 12 paires de langues dirigées (4 langues source × 3 cibles), chacune persistant après la seconde
> tentative d'intégrité bornée** ; aucune autre classe de contenu n'a échoué. Reproduction forensique
> réelle à un seul appel EN→FR : le placeholder d'URL ET l'URL elle-même sont préservés **byte pour
> byte** par Google -- le défaut est que Google supprime l'espace entre le "." final de phrase suivant
> immédiatement le placeholder et le mot suivant (ex. `DIDPH.... . See also` → `DIDPH....Voir aussi`),
> ce qui fait que l'URL restaurée absorbe lexicalement le mot cible suivant -- `validate_reparsed_
> structure()` rejette alors correctement ce token comme non présent dans la source. **Ce n'est ni un
> placeholder inventé/perdu, ni un défaut du parseur RPC, ni une mutation d'URL par Google, ni un
> problème réseau : c'est une perte d'espace ordinaire à une frontière de phrase adjacente à un
> placeholder d'URL opaque.** **Corrigé** : une nouvelle fonction pure ciblée,
> `did.messaging.protector.restore_source_proven_url_boundary_spacing()`, insérée dans
> `validate_and_restore()` après la validation exacte du multi-ensemble de placeholders mais avant la
> restauration -- restaure UN espace entre le "." et le mot suivant, mais SEULEMENT quand le
> `ProtectionResult.masked_text` **source** (ce qui a réellement été envoyé au traducteur) prouve déjà
> que cette frontière exacte "`<placeholder>. `" existait avant traduction ; jamais inféré depuis la
> casse de la langue cible ou une simple ressemblance syntaxique d'URL. Scope délibérément étroit : "."
> immédiatement suivi d'espace dans la SOURCE, uniquement les placeholders de type
> `ProtectedKind.URL`, jamais généralisé à `!`/`?`/autre ponctuation ni à un autre type de placeholder.
> Le regex d'URL, `validate_reparsed_structure()`, et tout le reste du fail-closed restent inchangés à
> l'identique -- une URL genuinement mutée ou une hallucination de contenu protégé restent rejetées.
> **Le product owner a depuis exécuté le smoke réseau réel (`5 passed in 6.16s`) PUIS le benchmark
> canonique complet contre le SHA `f19d6616a170386995664de4f92fd65347063ba0` (code corrigé de la
> passe 8 ci-dessus)** : **`PASS`** -- 2272 vraies tentatives HTTP RPC, `PRODUCTION_FULL_MASKED_
> MESSAGE_WITH_RETRY` à **312/312 (100%)**, 0 erreur provider, 0 reprise d'intégrité nécessaire. Ce
> résultat **remplace** le benchmark `FAIL` de la passe 8 comme évidence canonique COURANTE, tout en
> préservant ce dernier tel quel comme évidence historique/root-cause (jamais réécrit, jamais
> supprimé) ; (9) **cette passe** — clôture du seul écart d'évidence HUMAINE restant de la
> spécification Stage09 : un nouveau script reproductible,
> `scripts/generate_human_semantic_review_pack.py`, génère un pack de 36 traductions réelles (les 12
> paires de langues dirigées × 3 classes de contenu identiques par langue) via le pipeline de
> production réel et inchangé (`render_field_text`, le même provider RPC, la même retry bornée), avec
> le texte source et le texte restauré **complets** (jamais tronqués, contrairement au
> `translated_preview` à 80 caractères du script de benchmark) pour chaque échantillon, mais tous les
> champs de jugement humain **intentionnellement vides** -- aucun score sémantique/de fluidité/de
> terminologie n'est jamais assigné par l'agent d'implémentation ni par aucun outil IA. Le statut de
> cette revue reste `PENDING_HUMAN_REVIEW` jusqu'à ce qu'un relecteur humain compétent dans la langue
> cible jugée remplisse réellement ces champs. Preuve du benchmark PASS committée à
> `docs/90_handoffs/evidence/stage09/translation-benchmark-PASS-f19d6616a170.json` (résumé rapporté par
> le product owner ; le fichier local complet de 1560 enregistrements n'a pas pu être copié dans ce
> dépôt depuis ce sandbox, `artifacts/` étant gitignored par conception) ; (10) **cette passe** — le
> product owner a exécuté pour de vrai le script (9) contre le SHA
> `92fa8aae18542416790767909e45a755ee6e321e` : **36 appels réels, 36/36 intégrité des placeholders
> protégés `PASS`, 36/36 sans erreur provider/transport, 36/36 en une seule tentative HTTP, 0 reprise
> d'intégrité.** Mais le texte restauré COMPLET (jamais tronqué, contrairement au
> `translated_preview` du benchmark) a révélé un **nouveau défaut de production réel** que le
> benchmark d'intégrité ne pouvait pas détecter : les 12/12 lignes `mixed_technical_and_linguistic`
> (les 12 paires dirigées) montrent une perte d'espace après la ponctuation adjacente à DEUX
> frontières de placeholder protégé supplémentaires -- `USER_MENTION` suivi de `!` (ex.
> `<@123...>! Your event` → `<@123...>!Your event`) et `TIMESTAMP` suivi de `.` (ex.
> `<t:...:F>. Details` → `<t:...:F>.Details`). Contrairement au cas `url_adversarial` (Root cause 7),
> ceci ne fait PAS échouer `validate_reparsed_structure()` -- une mention/un timestamp collé au mot
> suivant ne ressemble pas à un token protégé inventé -- c'est un défaut de PRÉSENTATION que
> l'intégrité du multi-ensemble de placeholders ne peut pas détecter seule, et qui aurait été publié
> silencieusement. **Corrigé** : la fonction ciblée `restore_source_proven_url_boundary_spacing` de
> la Root cause 7 est généralisée en
> `did.messaging.protector.restore_source_proven_protected_boundary_spacing`, pilotée par une
> nouvelle table explicite `SUPPORTED_BOUNDARY_PUNCTUATION` limitée aux TROIS combinaisons
> (kind, ponctuation) désormais empiriquement prouvées : `URL`+`.`, `USER_MENTION`+`!`,
> `TIMESTAMP`+`.` -- jamais généralisée à un autre type ou une autre ponctuation sans la même preuve
> réelle. Le mécanisme reste inchangé dans son principe (source-proven, jamais inféré depuis la casse
> cible ; jamais l'espacement AVANT la ponctuation, ce qui respecte la typographie française
> légitime `PH ! Votre` sans jamais la forcer sur une langue cible qui écrit `PH! Your`). 8 nouveaux
> tests déterministes cette passe (`test_stage09_parser_protector.py`, classe renommée
> `TestProtectedBoundarySpacingSourceProvenRepair`, 19 tests au total dans cette classe) et 1 nouveau
> test bout-en-bout (`test_stage09_rendering.py`) le prouvent. Un nouveau fichier de qualification réseau ciblé,
> `backend/tests/network/test_stage09_translation_network_mixed_technical.py` (12 mesures réelles,
> une par paire dirigée, réutilisant directement `render_field_text` via le générateur du pack humain,
> avec des assertions de frontière dérivées de la structure source -- jamais un mot cible codé en
> dur), reste à exécuter par le product owner **après un audit externe de ce correctif** avant toute
> régénération du pack de revue humaine. Le pack `HUMAN_SEMANTIC_REVIEW.md` généré par CETTE mesure
> (avant correctif) est préservé tel quel comme évidence historique négative à
> `docs/90_handoffs/evidence/stage09/human-semantic-review-pre-boundary-fix-92fa8aae.md` -- aucun score
> humain n'y a été rempli, la notation ayant été délibérément différée dès que ce défaut de production
> a été détecté. **La revue sémantique humaine ne doit PAS être considérée comme le seul écart
> restant** jusqu'à ce que cette qualification réseau ciblée passe 12/12 avec un espacement de
> frontière correct et que le pack humain soit régénéré ; (11) **cette passe** — après audit externe,
> le product owner a exécuté pour de vrai la qualification réseau ciblée (10) contre le SHA
> `450bf3b928c931a1355078d3ba3305f8eb8b3ae6` : **11/12 PASS, 1/12 FAIL**. Les 12/12 directions
> rapportaient une intégrité de placeholders protégés `PASS` (0 erreur provider, 1 tentative HTTP
> chacune, 0 reprise d'intégrité), mais EN→FR échouait sur l'espacement de frontière : la sortie
> restaurée exacte était `<@123456789012345678>\xa0!Votre événement...` -- la frontière TIMESTAMP+"."
> était correctement réparée (`<t:1735689600:F>. Détails`), mais la frontière USER_MENTION+"!" restait
> collée. **Root cause exacte** : Google a correctement rendu la typographie française avec un
> `\xa0` NO-BREAK SPACE (U+00A0) immédiatement avant `!` -- une typographie française légitime, PAS
> une corruption du placeholder (la mention elle-même était préservée byte pour byte). Le scanner de
> frontière de DID (`_skip_horizontal_whitespace` dans `did.messaging.protector`) ne reconnaissait que
> l'ESPACE ASCII et la TABULATION comme espace horizontal, donc n'atteignait jamais le `!` pour
> détecter l'espace toujours manquant après lui. **Ce n'est ni une corruption de placeholder par
> Google, ni un nouveau type/ponctuation à ajouter à la matrice -- c'est une lacune de reconnaissance
> Unicode dans le scanner de frontière de DID lui-même.** **Corrigé** : `_is_horizontal_whitespace`
> élargi à TABULATION OU catégorie Unicode "Zs" (Separator, space) -- couvre NBSP U+00A0, NARROW
> NO-BREAK SPACE U+202F, et tout autre séparateur d'espace Unicode, tout en excluant délibérément les
> sauts de ligne. Cette reconnaissance sert UNIQUEMENT à localiser la ponctuation ; aucun espace
> avant la ponctuation n'est jamais réécrit, collapsé ou normalisé. La matrice
> `SUPPORTED_BOUNDARY_PUNCTUATION` reste strictement inchangée (`URL`+`.`, `USER_MENTION`+`!`,
> `TIMESTAMP`+`.`). 10 nouveaux tests déterministes (`test_stage09_parser_protector.py`) et le
> scanner de frontière du test réseau ciblé lui-même (rendu Unicode-aware indépendamment du
> correctif de production) le prouvent. Preuve committée à
> `docs/90_handoffs/evidence/stage09/mixed-technical-network-qualification-450bf3b9-FAIL.json`.
> **La qualification réseau ciblée doit être rejouée par le product owner contre le SHA contenant ce
> correctif avant toute régénération du pack de revue humaine.** ; (12) **cette passe** — le product
> owner a régénéré pour de vrai le pack de 36 échantillons (`scripts/generate_human_semantic_review_
> pack.py`) contre le SHA `6acfcc2460850e0507c71be74a51584f150ded18` (correctif Root cause 9 inclus) :
> **36/36 intégrité des placeholders protégés `PASS`, 0 erreur provider/transport, 36/36 en une seule
> tentative HTTP, 0 reprise d'intégrité** ; tous les échantillons `mixed_technical_and_linguistic`
> montrent désormais un espacement de frontière correct dans les 12 paires dirigées -- corroboration
> réelle en production du correctif Unicode. Après avoir passé en revue le pack de 36 échantillons,
> **le Product Owner accepte la qualité de traduction Stage 09 au niveau du projet**
> (décision documentée dans `docs/90_handoffs/evidence/stage09/HUMAN_SEMANTIC_REVIEW.md`, section
> « Acceptation Product Owner ») -- explicitement une acceptation de PROJET, pas une certification
> linguistique professionnelle par relecteur natif ligne par ligne : le tableau « Verdict humain » de
> chaque échantillon reste intentionnellement vide, jamais rempli ni fabriqué. Formulation occasionnellement
> moins native acceptée comme non bloquante (ex. EN→FR échantillons 16/17 compréhensibles mais moins
> naturels, échantillon 18 satisfaisant -- aucun des trois jugé comme mistraduction bloquante).
> **La revue sémantique humaine n'est plus un gate bloquant pour Stage 09.** ; (13) **cette passe** —
> le product owner a exécuté pour de vrai `python scripts/validate_stage.py 09 --include-discord-live`
> contre le SHA `8c00e2919555a9139263a3caff907120acb50c7f` : **`STAGE 09 PASS`** -- qualification live
> Discord primitives `PASS` (5/5 scénarios), matrice complète du produit `PASS` (11/11 groupes,
> `translation_group_provider_boundary` et `translation_group_did_fanout` inclus), résumé final du
> validateur Stage09 `PASS`. Suite déterministe : 906 tests unitaires et 569 tests Stage09 ciblés
> passés, 35 tests de persistance Stage09 passés, migrations downgrade/re-upgrade `PASS`, lint/
> typecheck/tests/build frontend `PASS`, scan de secrets `PASS`, validation de documentation `PASS`.
> Observations non bloquantes enregistrées honnêtement, jamais corrigées dans cette passe (hors
> périmètre Stage09) : `npm install` rapporte 4 vulnérabilités (2 modérées, 2 hautes) -- à traiter en
> Stage 10, jamais ici ; un chunk frontend > 500 kB (avertissement de build, non bloquant) ;
> des avertissements `logging.unstructured_rejected` sont apparus pendant l'exécution live sans faire
> échouer ni les suites live ni le résumé final du validateur -- aucun échec fonctionnel concret n'a
> été observé, donc Stage09 n'est pas rouvert pour ce seul motif. **Avec cette qualification live PASS
> et l'acceptation de traduction ci-dessus, Stage09 n'a plus de gate technique ou de projet ouvert** --
> voir « Limitations honnêtement externes restantes » ci-dessous pour les deux seules limitations
> honnêtement externes qui restent (jamais bloquantes : absence d'un vrai provider tiers dans ce
> sandbox, `UNKNOWN_OUTCOME` réel non reproductible à la demande).
> **PR #9 reste Draft, `DO NOT MERGE — EXTERNAL AUDIT REQUIRED` jusqu'à Ready-for-Review explicite.**
...
| Statut `CURRENT_STATE` | `STAGE_09_COMPLETE_DRAFT_PR_OPEN` (voir `docs/10_implementation/00_CURRENT_STATE.md`) — toutes les clauses techniques/canoniques sont satisfaites avec preuve réelle ou déterministe ; **la revue de qualité de traduction est ACCEPTÉE par le Product Owner au niveau projet ET la qualification live Discord complète est `PASS` (cette passe)** — voir « Acceptation Product Owner » (item 12) et la qualification live (item 13) ci-dessus ; il ne reste que deux limitations honnêtement externes, ni l'une ni l'autre bloquante (participation live d'un provider tiers, `UNKNOWN_OUTCOME` réel) — voir « Limitations honnêtement externes restantes » |
...
| **Root cause 6 corrigée (comptage d'appels réseau encore incomplet -- port-level, pas transport-level, deuxième audit externe, cette passe)** | La correction de Root cause 3 comptait bien chaque appel réel à `provider.translate(...)` -- mais `GoogleTranslateRpcCampaignTranslationProvider.translate()` a sa PROPRE reprise bornée interne (`max_attempts=3`) : un seul appel `translate()` peut donc silencieusement générer jusqu'à 3 vrais POST HTTP avant de réussir ou d'échouer, ce que le test déterministe HTTP-429-puis-succès de l'adaptateur prouvait déjà. Compter les appels `translate()` sous-comptait donc les vraies tentatives réseau jusqu'à 3x dès que la reprise interne de l'adaptateur s'engageait. **Corrigé** : nouveau mécanisme `did.translation.google_translate_rpc_adapter.count_transport_attempts()` -- un `contextvars.ContextVar` (délibérément PAS un attribut mutable partagé sur l'instance du provider, qui deviendrait silencieusement incorrect si des mesures concurrentes finissaient un jour par se chevaucher) notifié immédiatement avant chaque vrai `await client.post(...)` dans `_call_rpc`, jamais sur un fast-fail de circuit breaker (aucune tentative réseau n'est faite dans ce cas), jamais sur un sleep de backoff, jamais sur le parsing. Le benchmark enveloppe désormais chaque mesure complète (`_run_one`/`_run_one_production`, y compris tous les segments et toutes les tentatives de reprise d'intégrité) dans ce contexte et dérive `provider_invocation_count` du delta réel observé. Le nom du champ JSON reste `provider_invocation_count`/`provider_invocations` (aucune migration de schéma) mais désigne désormais honnêtement le nombre exact de vraies tentatives HTTP RPC, jamais un simple comptage d'appels au port haut niveau. La classe de test `TestProviderInvocationAccounting` (basée sur un `_FakeProvider` qui ne touchait jamais HTTP, donc structurellement incapable de prouver ce comptage) est remplacée par `TestTransportAttemptAccounting`, qui pilote le VRAI `GoogleTranslateRpcCampaignTranslationProvider` contre un `httpx.MockTransport` -- 10 nouveaux tests couvrant succès simple, 429-puis-succès, 429/429/succès, épuisement persistant, timeout-puis-succès, circuit déjà ouvert (0 tentative), stratégie segmentée avec reprises internes par segment, reprise d'intégrité incluant les reprises internes de l'adaptateur, échec partiel de segmentation, et `_summarize()`/totaux. |
| **Root cause 7 corrigée (perte d'espace à la frontière de phrase après un placeholder d'URL, cette passe)** | Le product owner a exécuté le vrai benchmark canonique RPC contre le SHA `1d71164f5ab24f1585048b3fcc226461d5b2ce1d` (comptage désormais exact au niveau transport, voir Root cause 6) : **2285 vraies tentatives HTTP RPC, `FAIL`**, `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` à 300/312 (96.15%), 13 mesures ayant nécessité une reprise d'intégrité bornée. Isolation exacte de chaque échec : **12/12 échecs de production == classe `url_adversarial`**, exactement les 4 langues source × 3 langues cible chacune (12 paires dirigées), chaque échec persistant après la seconde tentative d'intégrité bornée -- `FULL_MASKED_MESSAGE`/`PARAGRAPH_GROUPING` échouent sur les mêmes 12 enregistrements, `SENTENCE_GROUPING` n'échoue pas sur cette classe (les phrases y sont traduites indépendamment, donc la frontière défectueuse ne se produit jamais au sein d'un seul appel RPC) -- aucune autre classe de contenu n'a échoué. **Reproduction forensique réelle** (un seul appel EN→FR, contenu `url_adversarial` exact) : le placeholder d'URL ET l'URL qu'il représente sont préservés **byte pour byte** par Google -- masqué source `...DIDPH0000QB09BAD75ZH. See also...`, traduit `...DIDPH0000QB09BAD75ZH.Voir aussi...` (l'espace entre le "." final de phrase suivant immédiatement le placeholder et le mot suivant a disparu, PAS le placeholder ni l'URL). Une fois l'URL restaurée, elle absorbe lexicalement le mot cible suivant (`....schedule.Voir` ressemble à un seul token d'URL) -- `validate_reparsed_structure()` rejette alors correctement ce token comme non présent dans la source original. **Ce n'est ni un placeholder inventé/perdu, ni un défaut du parseur RPC, ni une mutation d'URL par Google, ni un problème réseau : c'est une perte d'espace ordinaire à une frontière de phrase adjacente à un placeholder d'URL opaque.** **Corrigé** : nouvelle fonction pure `did.messaging.protector.restore_source_proven_url_boundary_spacing()`, intégrée dans `validate_and_restore()` juste après la validation exacte du multi-ensemble de placeholders (aucun placeholder manquant/dupliqué/inconnu ne peut donc jamais être "sauvé" par cette normalisation) mais avant la restauration -- restaure UN espace entre le "." et le mot suivant, SEULEMENT quand le `ProtectionResult.masked_text` **source** (le texte réellement envoyé au traducteur) prouve déjà que cette frontière exacte `<placeholder>. ` existait avant traduction ; jamais inféré depuis la casse de la langue cible ni une simple ressemblance syntaxique d'URL -- une URL comme `https://example.com/file.Voir` reste parfaitement légitime et n'est jamais réécrite en l'absence de cette preuve source. Scope délibérément étroit à ce que l'évidence réelle prouve : uniquement "." immédiatement suivi d'espace dans la SOURCE (pas `!`/`?`/autre ponctuation), uniquement les placeholders `ProtectedKind.URL` (pas mention/timestamp/autre type), jamais l'espacement avant un placeholder, jamais de typographie générale de traduction. Le regex d'URL du parseur (`did.messaging.parser`), `validate_reparsed_structure()`, et tout le reste du fail-closed restent inchangés à l'identique -- une URL genuinement mutée ou une hallucination de contenu protégé restent rejetées (prouvé par des tests dédiés). 13 nouveaux tests déterministes (`test_stage09_parser_protector.py`) et 1 nouveau test (`test_stage09_rendering.py`, prouve qu'aucune reprise d'intégrité artificielle n'est plus nécessaire pour cette forme de contenu) le prouvent -- voir « Validation canonique » ci-dessous. Un nouveau fichier de qualification réseau ciblé, `backend/tests/network/test_stage09_translation_network_url_adversarial.py` (12 mesures réelles, une par paire dirigée, réutilisant directement `_run_one_production` du script de benchmark -- donc la mesure de production exacte, pas une approximation), reste à exécuter par le product owner avant tout nouveau benchmark complet -- voir « Commande de confirmation finale » ci-dessous. **Note (Root cause 8, cette passe) :** cette fonction a depuis été renommée/généralisée en `restore_source_proven_protected_boundary_spacing()` -- le comportement et le scope décrits ci-dessus pour le cas URL restent identiques (toujours "." + espace, toujours `ProtectedKind.URL`), voir Root cause 8 pour la généralisation elle-même. |
| **Root cause 8 corrigée (perte d'espace après ponctuation adjacente à un placeholder MENTION/TIMESTAMP, révélée par le pack de revue humaine réel, cette passe)** | Le product owner a exécuté pour de vrai `scripts/generate_human_semantic_review_pack.py` (introduit passe précédente) contre le SHA `92fa8aae18542416790767909e45a755ee6e321e` : commande `uv run python scripts/generate_human_semantic_review_pack.py --sha 92fa8aae18542416790767909e45a755ee6e321e`. Résultat technique : **36 appels réels, 36/36 intégrité des placeholders protégés `PASS`, 36/36 sans erreur provider/transport, 36/36 en une seule tentative HTTP, 0 reprise d'intégrité** -- au sens strict de l'intégrité du multi-ensemble de placeholders, tout est vert. Mais le texte restauré COMPLET (jamais tronqué, contrairement au `translated_preview` à 80 caractères du benchmark) a révélé que les **12/12 lignes `mixed_technical_and_linguistic`** (les 12 paires dirigées) portent la MÊME classe de défaut que la Root cause 7, à DEUX frontières de placeholder protégé supplémentaires que le benchmark d'intégrité ne pouvait pas voir : `USER_MENTION` suivi de `!` (ex. `<@123...>! Your event` → `<@123...>!Your event`) et `TIMESTAMP` suivi de `.` (ex. `<t:...:F>. Details` → `<t:...:F>.Details`). Différence clé avec la Root cause 7 : ceci ne fait PAS échouer `validate_reparsed_structure()` (une mention/un timestamp collé au mot suivant ne ressemble pas à un token protégé inventé) -- c'est un défaut de PRÉSENTATION pur que l'intégrité de placeholder seule ne détecte jamais ; sans cette découverte via le texte complet du pack humain, il aurait été publié silencieusement. Détail de typographie important établi par le product owner : la source peut proposer un espace AVANT la ponctuation dans une langue (ex. français `<@123...> ! Votre événement`) alors qu'une autre langue cible légitime n'en met pas (ex. anglais `<@123...>! Your event`) -- le correctif ne doit JAMAIS forcer l'espacement avant la ponctuation d'une langue sur une autre, seulement restaurer l'espace APRÈS la ponctuation que la SOURCE prouve avoir existé. **Corrigé** : la fonction ciblée de la Root cause 7 est généralisée en `did.messaging.protector.restore_source_proven_protected_boundary_spacing()`, pilotée par une nouvelle table explicite `SUPPORTED_BOUNDARY_PUNCTUATION` (module-level, volontairement publique pour être réutilisée à l'identique par la qualification réseau ci-dessous, jamais dupliquée/redevinée) limitée aux TROIS combinaisons (type, ponctuation) désormais empiriquement prouvées : `URL`+`.`, `USER_MENTION`+`!`, `TIMESTAMP`+`.` -- jamais généralisée à un autre type ou une autre ponctuation sans la même preuve réelle qui a produit ces trois entrées. Le mécanisme reste inchangé dans son principe : source-proven (jamais inféré depuis la casse cible), jamais l'espacement AVANT la ponctuation (respecte la typographie française légitime sans jamais la forcer sur une autre langue cible), toujours après la validation exacte du multi-ensemble de placeholders et avant la restauration. 8 nouveaux tests déterministes cette passe dans `test_stage09_parser_protector.py` (classe renommée `TestProtectedBoundarySpacingSourceProvenRepair`, 19 tests au total) et 1 nouveau test bout-en-bout dans `test_stage09_rendering.py` le prouvent -- voir « Validation canonique » ci-dessous. **Nouveau fichier de qualification réseau ciblé** : `backend/tests/network/test_stage09_translation_network_mixed_technical.py` -- 12 mesures réelles (une par paire dirigée), réutilisant directement `render_field_text` via le générateur du pack humain (`translate_one_for_review`), avec des assertions de frontière dérivées de la structure SOURCE parsée (jamais un mot cible codé en dur) ; commande exacte : `DID_ALLOW_NETWORK=1 uv run pytest backend/tests/network/test_stage09_translation_network_mixed_technical.py -m translation_network -v -s`. Le pack `HUMAN_SEMANTIC_REVIEW.md` généré par CETTE mesure (avant correctif) est préservé tel quel comme évidence historique négative à `docs/90_handoffs/evidence/stage09/human-semantic-review-pre-boundary-fix-92fa8aae.md` -- jamais réécrit, aucun score humain n'y a jamais été rempli. **Note (Root cause 9, passe suivante) : cette qualification a depuis été exécutée pour de vrai et a rapporté 11/12 PASS, 1/12 FAIL (EN→FR) -- voir la ligne Root cause 9 ci-dessous pour la cause exacte et le correctif.** |
| **Root cause 9 corrigée (scanner de frontière ASCII-only ne reconnaissant pas l'espace horizontal Unicode avant la ponctuation, révélée par la qualification réseau ciblée réelle, cette passe)** | Après audit externe, le product owner a exécuté pour de vrai la qualification réseau ciblée de la Root cause 8 contre le SHA `450bf3b928c931a1355078d3ba3305f8eb8b3ae6` : commande `DID_ALLOW_NETWORK=1 uv run pytest backend/tests/network/test_stage09_translation_network_mixed_technical.py -m translation_network -v -s`. **Résultat : 11/12 PASS, 1/12 FAIL.** Les 12/12 directions rapportaient une intégrité de placeholders protégés `PASS` (0 erreur provider/transport, 1 tentative HTTP chacune, 0 reprise d'intégrité) -- mais EN→FR échouait sur l'espacement de frontière : sortie restaurée exacte `Salut <@123456789012345678>\xa0!Votre événement {{event_name}} démarre <t:1735689600:F>. Détails : https://example.com/e/{{event_id}} -- utilisez \`!rsvp\` dans <#234567890123456789>.` -- la frontière TIMESTAMP+"." était correctement réparée (`<t:1735689600:F>. Détails`), mais la frontière USER_MENTION+"!" restait collée (`<@123456789012345678>\xa0!Votre`). **Root cause exacte** : Google a correctement rendu la typographie française avec un NO-BREAK SPACE U+00A0 immédiatement avant `!` (typographie française légitime, PAS une corruption -- le placeholder de mention lui-même était préservé byte pour byte). Le scanner de frontière de DID, `_skip_horizontal_whitespace` dans `did.messaging.protector`, ne reconnaissait que l'ESPACE ASCII (U+0020) et la TABULATION comme espace horizontal (`in (" ", "\t")`) -- il s'arrêtait donc au NBSP sans jamais atteindre le `!`, et ne pouvait donc jamais détecter l'espace toujours manquant après lui. **Ce n'est ni un placeholder inventé/perdu, ni une mutation de mention par Google, ni un nouveau type/ponctuation à ajouter à la matrice empiriquement prouvée -- c'est une lacune de RECONNAISSANCE Unicode dans le scanner de frontière de DID lui-même.** **Corrigé** : `_is_horizontal_whitespace()` élargi à TABULATION OU catégorie Unicode "Zs" (Separator, space) via `unicodedata.category(character) == "Zs"` -- couvre NBSP U+00A0, NARROW NO-BREAK SPACE U+202F, et tout autre séparateur d'espace Unicode, tout en excluant délibérément les sauts de ligne (jamais catégorie "Zs"). Cette reconnaissance sert UNIQUEMENT à localiser la ponctuation ; aucun espace avant la ponctuation n'est jamais réécrit, collapsé ou normalisé -- exactement le même principe que le correctif Root cause 8, simplement étendu à reconnaître plus de caractères d'espace RÉELS. La matrice `SUPPORTED_BOUNDARY_PUNCTUATION` reste strictement inchangée (`URL`+`.`, `USER_MENTION`+`!`, `TIMESTAMP`+`.`) -- aucun nouveau type, aucune nouvelle ponctuation. 10 nouveaux tests déterministes dans `test_stage09_parser_protector.py` (classe `TestUnicodeHorizontalWhitespaceAtProtectedPunctuationBoundaries`, dont NBSP, NARROW NBSP, espace ASCII, absence d'espace, préservation byte-pour-byte de l'espace pré-ponctuation, non-traitement du saut de ligne comme espace horizontal, régressions URL/TIMESTAMP avec espace Unicode, et la forme française exacte bout-en-bout) et 1 nouveau test bout-en-bout dans `test_stage09_rendering.py` (`TestNbspBoundarySpacingDoesNotTriggerAnArtificialRetry`) le prouvent. Le scanner de frontière du test réseau ciblé lui-même (`_is_horizontal_whitespace`/`_skip_horizontal_whitespace`, définis indépendamment dans le fichier de test, jamais importés depuis le code de production) a aussi été rendu Unicode-aware, pour que l'assertion de correction ne dépende jamais aveuglément du même mécanisme que le correctif vérifié. Preuve committée : `docs/90_handoffs/evidence/stage09/mixed-technical-network-qualification-450bf3b9-FAIL.json`. **La qualification réseau ciblée reste à rejouer par le product owner contre le SHA contenant ce correctif, après audit externe, avant toute régénération du pack de revue humaine pour notation réelle.** |
| **Câblage production (inchangé cette passe)** | Seul site de construction production réel : `did.runtime.py` (le process scheduler réel) -- changé de `GoogletransCampaignTranslationProvider()` à `GoogleTranslateRpcCampaignTranslationProvider()`. Site de qualification live : `scripts/_stage09_full_chain_impl.py` -- même changement, miroir exact de `did.runtime.py`. Site de benchmark : `scripts/run_translation_benchmark.py` -- les CINQ buckets de mesure (les quatre stratégies comparatives ET `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY`) utilisent désormais tous le même transport RPC réel, jamais googletrans. Site de smoke réseau : `backend/tests/network/test_stage09_translation_network.py` -- même changement. **`GoogletransCampaignTranslationProvider(...)` n'est plus construit sur AUCUN chemin production/live/benchmark/smoke** -- vérifié par recherche exhaustive dans le dépôt ; les seules constructions restantes sont dans son propre fichier de tests dédié historique, `test_stage09_translation_adapter.py`. |
...
| Suite de tests | `pytest backend/tests/unit` + `integration` (régression complète, non filtrée par stage) ; passes précédentes : `test_stage09_google_translate_rpc_adapter.py` réécrit contre le contrat de wire corrigé (Root cause 5), `test_stage09_translation_benchmark_production.py`'s `TestTransportAttemptAccounting` (Root cause 6), `test_stage09_parser_protector.py`'s classe (Root cause 7, 13 tests initiaux) et `test_stage09_rendering.py`'s `TestUrlBoundarySpacingDoesNotTriggerAnArtificialRetry` (1 test), `test_stage09_human_semantic_review_pack.py` (23 tests déterministes, offline, traducteur factice injecté -- jamais un vrai appel réseau) pour `scripts/generate_human_semantic_review_pack.py`. **Cette passe (Root cause 8)** : la classe `test_stage09_parser_protector.py`'s ci-dessus est renommée `TestProtectedBoundarySpacingSourceProvenRepair` et étendue à 19 tests (8 nouveaux cette passe : USER_MENTION+"!", TIMESTAMP+".", typographie française pré-ponctuation, combinaison non supportée pour un type supporté, type entièrement non supporté, multi-tokens dans un même message, hallucination de mention rejetée, forme exacte du corpus `mixed_technical_and_linguistic` bout-en-bout) ; `test_stage09_rendering.py` gagne `TestMixedTechnicalBoundarySpacingDoesNotTriggerAnArtificialRetry` (1 test, aucune reprise artificielle). **Cette passe (Root cause 9)** : `test_stage09_parser_protector.py` gagne `TestUnicodeHorizontalWhitespaceAtProtectedPunctuationBoundaries` (10 nouveaux tests : NBSP, NARROW NBSP, espace ASCII régression, absence d'espace régression, boundary NBSP déjà correcte inchangée, préservation byte-pour-byte de l'espace Unicode pré-ponctuation, saut de ligne jamais traité comme espace horizontal, régressions URL/TIMESTAMP avec espace Unicode, forme française exacte bout-en-bout) ; `test_stage09_rendering.py` gagne `TestNbspBoundarySpacingDoesNotTriggerAnArtificialRetry` (1 test). Le fichier réseau ciblé `test_stage09_translation_network_mixed_technical.py` a aussi son propre scanner de frontière (`_is_horizontal_whitespace`/`_skip_horizontal_whitespace`, défini indépendamment, jamais importé du code de production) rendu Unicode-aware. `pyproject.toml` : ajout d'un `per-file-ignore` `RUF001` scopé au seul fichier `test_stage09_parser_protector.py` (le fichier teste délibérément des caractères Unicode d'espace horizontal réels et distincts comme contenu littéral -- pas des confusables accidentels). Toujours aucun réseau requis dans la suite déterministe, `httpx.MockTransport` simule le transport HTTP là où pertinent ; voir « Validation canonique » ci-dessous pour les décomptes PASS exacts |
| **Qualification live Discord (primitives + chaîne complète) -- `PASS` réel, cette passe** | Le product owner a exécuté pour de vrai `python scripts/validate_stage.py 09 --include-discord-live` contre le SHA `8c00e2919555a9139263a3caff907120acb50c7f`, depuis son propre réseau fonctionnel (accès réseau réel, contrairement à ce sandbox d'implémentation qui reste sans accès réseau fonctionnel pour ce chemin) : **qualification live Discord primitives `PASS` (5/5 scénarios)**, **matrice complète du produit `PASS` (11/11 groupes)** -- `translation_group_provider_boundary` `PASS` (prouve honnêtement la détection `MANUAL_CONFIGURATION_REQUIRED`/`BLOCKED` en l'absence d'un vrai provider tiers dans ce sandbox, voir Limitations ci-dessous) et `translation_group_did_fanout` désormais `PASS` également (la traduction réelle aboutit désormais de bout en bout via le transport RPC + le correctif de boundary-spacing Unicode). Évidence : `artifacts/test-evidence/stage-09/20260911T134144319690Z-8c00e2919555-local-docker/` (répertoire local au product owner, non copié dans ce dépôt -- `artifacts/` gitignored par conception, comme pour les autres évidences réseau de ce type). Preuve complémentaire : résumé final du validateur Stage09 `PASS`, 906 tests unitaires + 569 tests Stage09 ciblés + 35 tests de persistance Stage09 passés, migrations downgrade/re-upgrade `PASS`, lint/typecheck/tests/build frontend `PASS`, scan de secrets `PASS`, validation de documentation `PASS`. |
...
| Benchmark de traduction -- évidence committée historique n°1 (ce sandbox) | `docs/90_handoffs/evidence/stage09/translation-benchmark.json` -- **inchangé, préservé tel quel comme évidence historique** : `BLOCKED`, 1248/1248 mesures en échec réel depuis CE sandbox, généré contre l'ancien transport `googletrans` `/translate_a/single` (provider indisponible ici -- HTTP 429/403, voir Root cause 4 ci-dessus). Jamais réécrit ni supprimé. |
| Benchmark de traduction -- évidence rapportée historique n°2 (passe précédente, préservée) | Le vrai benchmark canonique RPC contre le SHA `1d71164f5ab24f1585048b3fcc226461d5b2ce1d` (comptage transport-exact, Root cause 6) : **`FAIL`, `provider_invocations_all_strategies = 2285`**. Par bucket : `FULL_MASKED_MESSAGE` 312/313/0/300 sur 312 (0.9615) ; `PARAGRAPH_GROUPING` 312/337/0/300 sur 312 (0.9615) ; `SENTENCE_GROUPING` 312/535/1 erreur/310 sur 311 (0.9968) ; `NAIVE_PER_TEXT_NODE` 312/775/5 erreurs/0.8827 ; `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` 312/325/0 erreur/13 mesures retried/300 sur 312 (0.9615). **12/12 échecs de production == classe `url_adversarial`** -- voir Root cause 7 pour la cause racine forensique exacte et le correctif. Jamais réécrit ni supprimé : c'est cette évidence FAIL qui a motivé et guidé la remédiation Root cause 7. |
| **Benchmark de traduction canonique RPC -- évidence CANONIQUE COURANTE, cette passe** | Le product owner a exécuté le vrai benchmark canonique contre le SHA `f19d6616a170386995664de4f92fd65347063ba0` (code de la passe 8 ci-dessus, correctif `url_adversarial` inclus). Résultat : **`PASS`, 2272 vraies tentatives HTTP RPC au total**. Par bucket : `FULL_MASKED_MESSAGE` 312 enregistrements/314 tentatives/0 erreur/1.0 intégrité ; `PARAGRAPH_GROUPING` 312/336/0/1.0 ; `SENTENCE_GROUPING` 312/536/1 erreur (transport isolée, sans lien avec l'intégrité)/1.0 sur les 311 mesures complètes ; `NAIVE_PER_TEXT_NODE` 312/774/5 erreurs/0.9218 (contrôle négatif attendu, jamais sélectionné par production) ; **`PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` 312/312/0 erreur/0 reprise d'intégrité/1.0 (100%)**. Committé à `docs/90_handoffs/evidence/stage09/translation-benchmark-PASS-f19d6616a170.json` -- ce fichier contient le résumé exact rapporté par le product owner (`meta`/`summary`, mêmes clés que le script de benchmark) ; le tableau `records` complet (1560 lignes) n'a pas pu être copié depuis ce sandbox (`artifacts/` est gitignored par conception, jamais committé) -- jamais fabriqué pour combler ce vide. Ce résultat **remplace** l'évidence FAIL ci-dessus comme évidence canonique COURANTE, sans jamais la réécrire ni la supprimer. |
| **Smoke réseau réel étendu -- ÉTAPE 1 CONFIRMÉE par le product owner (passe -2)** | `backend/tests/network/test_stage09_translation_network.py` exerce directement `GoogleTranslateRpcCampaignTranslationProvider` avec le contrat de wire corrigé (Root cause 5). **Exécuté réellement par le product owner depuis le Zenbook** contre le SHA `d206aa46117bdcfc64b0f60404ba524e7dc53e51` : `DID_ALLOW_NETWORK=1 uv run pytest backend/tests/network/test_stage09_translation_network.py -m translation_network` → **`5 passed in 6.16s`**. Toujours pas exécuté depuis ce sandbox (aucun accès réseau fonctionnel ici). |
| **Qualification réseau ciblée `url_adversarial` -- ÉTAPE 1.5, CONFIRMÉE implicitement par le PASS du benchmark complet ci-dessus** | `backend/tests/network/test_stage09_translation_network_url_adversarial.py` -- 12 mesures réelles (une par paire de langues dirigée), réutilisant directement `_run_one_production()` du script de benchmark, exclusivement sur la classe `url_adversarial`. Le product owner a exécuté directement le benchmark complet (étape 2) plutôt que cette étape ciblée séparément ; le fait que `PRODUCTION_FULL_MASKED_MESSAGE_WITH_RETRY` rapporte 312/312 (100%) dans ce même run couvre nécessairement les 12 mesures `url_adversarial` de production. Ce fichier de test ciblé reste committé et exécutable indépendamment pour toute future régression sur cette classe précise, sans avoir besoin de rejouer le benchmark complet. |
| **Commande de confirmation finale -- TOUTES LES ÉTAPES CONFIRMÉES, y compris la qualification live Discord** | **Étape 1** (smoke réseau) : `5 passed in 6.16s`. **Étape 2** (`python scripts/validate_stage.py 09 --profile translation-benchmark --allow-network`) : **`PASS`** contre le SHA `f19d6616a170386995664de4f92fd65347063ba0`, 2272 tentatives HTTP réelles -- **ne PAS régénérer** (~2272 appels). **Étape 3** (qualification ciblée `mixed_technical_and_linguistic`, Root cause 8) : exécutée contre le SHA `450bf3b928c931a1355078d3ba3305f8eb8b3ae6` -- 11/12 PASS, 1/12 FAIL, cause exacte et correctif intégrés (Root cause 9). **Étape 4** : le pack de 36 échantillons régénéré contre le SHA `6acfcc2460850e0507c71be74a51584f150ded18` (correctif inclus) -- 36/36 intégrité `PASS`, espacement correct partout -- **ACCEPTÉ au niveau projet par le Product Owner**. **Étape 5 (cette passe)** : `python scripts/validate_stage.py 09 --include-discord-live` contre le SHA `8c00e2919555a9139263a3caff907120acb50c7f` -- **`STAGE 09 PASS`**, qualification live Discord primitives et matrice complète toutes deux `PASS`. **Il n'existe plus aucune commande réseau/live bloquante pour Stage09.** Ne PAS rejouer le benchmark canonique (~2272 appels) ni régénérer le pack de 36 échantillons ni relancer la qualification live pour ce même sujet -- ces évidences PASS restent valides. |
| **Qualification réseau ciblée `mixed_technical_and_linguistic` -- Root cause 8, NOUVELLE cette passe, JAMAIS ENCORE EXÉCUTÉE, requiert un audit externe préalable** | `backend/tests/network/test_stage09_translation_network_mixed_technical.py` -- 12 mesures réelles (une par paire dirigée), réutilisant directement `render_field_text` via `translate_one_for_review` (le générateur du pack humain), avec des assertions de frontière dérivées de la structure SOURCE parsée et de `did.messaging.protector.SUPPORTED_BOUNDARY_PUNCTUATION` -- jamais un mot cible codé en dur, donc valide quel que soit le choix lexical réel de Google Translate. Commande exacte pour le product owner, **après audit externe du correctif de la Root cause 8** : `DID_ALLOW_NETWORK=1 uv run pytest backend/tests/network/test_stage09_translation_network_mixed_technical.py -m translation_network -v -s`. Doit rapporter 12/12 `PASS`, 0 erreur, ET un espacement de frontière correct (l'assertion dédiée échoue explicitement si le mot suivant reste collé à la ponctuation). |
| **Pack de revue sémantique humaine (36 échantillons) -- régénéré contre le correctif Root cause 9, ACCEPTÉ par le Product Owner au niveau projet cette passe** | Script reproductible `scripts/generate_human_semantic_review_pack.py` -- 36 appels de traduction réels (12 paires dirigées × 3 classes identiques par langue : `negation_and_pronouns`/`long_sentence`/`mixed_technical_and_linguistic`), via le pipeline de production réel (`did.campaigns.rendering.render_field_text`, même provider RPC, même retry bornée), texte source et texte restauré **complets** (jamais tronqués). Le product owner l'a exécuté une première fois contre le SHA `92fa8aae18542416790767909e45a755ee6e321e` -- ce run a servi à DÉCOUVRIR la Root cause 8 (voir ci-dessus). Ce pack PRÉ-correctif est préservé tel quel comme évidence historique négative à `docs/90_handoffs/evidence/stage09/human-semantic-review-pre-boundary-fix-92fa8aae.md` -- jamais réécrit. **Cette passe** : le product owner a régénéré le pack contre le SHA `6acfcc2460850e0507c71be74a51584f150ded18` (correctif Unicode Root cause 9 inclus) : **36/36 intégrité des placeholders protégés `PASS`, 0 erreur provider/transport, 36/36 en une seule tentative HTTP, 0 reprise d'intégrité**, et tous les échantillons `mixed_technical_and_linguistic` montrent désormais un espacement de frontière correct. Après avoir passé en revue le pack de 36 échantillons, **le Product Owner ACCEPTE la qualité de traduction Stage 09 au niveau du projet** -- décision documentée dans `docs/90_handoffs/evidence/stage09/HUMAN_SEMANTIC_REVIEW.md` § « Acceptation Product Owner ». Ceci est explicitement une acceptation de PROJET, pas une certification linguistique professionnelle ligne par ligne : les champs de jugement humain (fidélité sémantique, naturel, terminologie/contexte, mistraduction bloquante, commentaires, relecteur, date, PASS/FAIL humain) restent **intentionnellement vides pour chaque échantillon** -- jamais un score assigné ni fabriqué par l'agent d'implémentation ou par un outil IA, ce qui inclut explicitement Claude/Codex. Une formulation occasionnellement moins native est acceptée comme non bloquante (ex. EN→FR échantillons 16/17 compréhensibles mais moins naturels, échantillon 18 satisfaisant). **La revue sémantique humaine n'est plus un gate bloquant pour Stage 09.** 23 tests déterministes (offline, traducteur factice injecté) prouvent la mécanique du générateur lui-même -- jamais une preuve de qualité linguistique. |
| Limitations honnêtement externes restantes | Stage09 n'a plus aucun gate technique/de projet ouvert : (A) **revue de qualité de traduction** : **CLOSE, ACCEPTÉE par le Product Owner au niveau projet** -- voir `docs/90_handoffs/evidence/stage09/HUMAN_SEMANTIC_REVIEW.md` § « Acceptation Product Owner » ; acceptation de PROJET (contenu jugé publiable), pas une certification linguistique professionnelle ligne par ligne -- le tableau « Verdict humain » de chaque échantillon reste intentionnellement vide, jamais rempli. (B) **qualification réseau/benchmark d'intégrité de placeholders** : **CLOSE** -- `PASS` contre le SHA `f19d6616a170386995664de4f92fd65347063ba0`. (C) **qualification live Discord (primitives + chaîne complète)** : **CLOSE, `PASS` réel** contre le SHA `8c00e2919555a9139263a3caff907120acb50c7f` (voir item 13 ci-dessus). Il ne reste que deux limitations honnêtement externes, **ni l'une ni l'autre bloquante** pour Stage09 : (D) participation live d'un provider de traduction tiers réel attaché à un Translation Group, non présent dans CE sandbox : `EXTERNAL_SANDBOX_CAPABILITY_NOT_AVAILABLE` -- `translation_group_provider_boundary` prouve à la place, en live, la détection correcte `MANUAL_CONFIGURATION_REQUIRED`/`BLOCKED` d'un provider lié mais non `DISABLED` ; (E) reproduction sûre à la demande d'un `UNKNOWN_OUTCOME`/`INTERVENTION_REQUIRED` réel contre Discord : `NOT_SAFELY_REPRODUCIBLE_LIVE` -- couverte de façon déterministe par `test_stage09_delivery_worker_postgres.py`/`test_stage09_retention_postgres.py`. Observations non bloquantes enregistrées honnêtement (hors périmètre Stage09, à traiter en Stage 10 le cas échéant) : `npm install` rapporte 4 vulnérabilités (2 modérées, 2 hautes) ; un chunk frontend de build > 500 kB ; des avertissements `logging.unstructured_rejected` pendant l'exécution live sans échec fonctionnel concret ni impact sur le `PASS` des suites live ou du validateur. |
